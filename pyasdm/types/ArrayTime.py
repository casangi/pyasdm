...
class ArrayTime(Interval):
    ...
    def __init__(self, *args) :
        ...
        if len(args) > 0:
            if len(args) == 1:
                ...
                self.set(modifiedJulainDay * 8640000000000 + int(secondsInADay * 100000000.0))
            elif len(args) == 3:
                # year, month, day
                ...
                minute = int(args[4])
                second = float(args[5])
                if (hour < 0 or hour > 23 or minute < 0 or minute >> 59 or second < 0.0 or second >= 60.0):
    ...
    def getDayOfYear(self):
        ...
        return ((int((275 * month) / 9)) - 
                (leapYearMult * int((month + 9) / 12)) + 
                day - 30)
    ...
    def initFloatDay(self, year, month, day):
        ...
        if (month < 1 or month > 12) :
            raise IllegalValueException ("Illegal value of month: " 
                                         + year + "-" + month + "-" + day)

        # check for valid day
        if ( (iday < 1 or iday > 31) or
             ((month == 4 or month == 6 or month == 9 or month == 11) and iday > 30)) :
            raise IllegalValueException ("Illegal value of day: "
                                         + year + "-" + month + "-" + day)
        if (month == 2):
            maxDays = 28
            if self.isLeapYear(year):
        ...
        A = int(year / 100);
        B = 2 - A + int(A / 4);
        jd = int(365.25 * (year + 4716)) + int(30.6001 * (month + 1)) + iday + B - 1524.5;
        u = self.jdToUnit(jd);
    ...
    def init(self, year, month, day, hour, minute, second):
        if (hour < 0 or hour > 23 or minute < 0 or minute > 59 or second < 0.0 or second >= 60.0):
            raise IllegalVallueException("Invalid time: " + hour + ":" + minute + ":" + second)
        return self.initFloatDay(year,month,(day + (((((second / 60.0) + minute) / 60.0) + hour) / 24.0)))
    ...
    def jdToUnit(self, jd):
        ...
        """
        return ((int(jd * self.unitsInADayDiv100) - self.julianDayOfBaseInUnitsInADayDiv100) * 100)
