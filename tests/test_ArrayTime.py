...
class arraytime_test(unittest.TestCase):
    ...
    def test_known_results(self):
        ...
        self.assertAlmostEqual(
            at.getLocalSiderealTime(-5.0),
            19.2413337437319569,
            18,
            "getLocalSiderealTime",
        )
        self.assertAlmostEqual(
            at.getGreenwichMeanSiderealTime(),
            14.2413337437319569,
            18,
    ...
    def test_constructors(self):
        ...
        self.assertTrue(
            (abs(at.get() - self.atnow.get()) < 1000), "ArrayTime from a string MJD"
        )
        ...
        at = ArrayTime(
            vDateTime[0], vDateTime[1], vDateTime[2], vDateTime[3], vDateTime[4], fsec
        )
        ...
        fday = (
            ...
            + (
                vDateTime[3]
                + (vDateTime[4] + (vDateTime[5] + vDateTime[6] / 1000000000.0) / 60.0)
                / 60.0
            )
            / 24.0
        )
        ...
        self.assertTrue(
            (abs(at.get() - self.atnow.get()) < 1000), "ArrayTime from MJD (float)"
        )

        # a non-leap year, to check day of year and initialization
        at = ArrayTime(1983, 7, 14, 22, 2, 5.7528)
        self.assertEqual(
            at.toFITS(), "1983-07-14T22:02:05.752800000", "non leap year test, toFITS"
        )
        ...
        self.assertEqual(
            at.toFITS(),
            "2012-02-22T22:30:50.000136000",
            "FITS date in February test, toFITS",
        )
