...
def benchmarkTable(container, tableName, tableNameWidth, checkRowUniqueness):
    ...
    print(
        f"{tableName:^{tableNameWidth}} {tableType:>4} {thisTable.size():>6} {container.getExpectedTableSize(tableName):>6} {strFileSize:>10} {sizeTable:>11} {deltaGetTableTime:08.4f} {timePerRow_in_ms:07.3f}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark ASDM memory and load times by table."
    )
    ...
    try:
        ...
        print(
            f"Conversion Exception seen when setting sdm to path: {sdmPath}. Exception : {str(e)}"
        )
        sys.exit(1)
    except Exception as e:
        print(
            f"Unknown exception seen when setting the sdm to path: {sdmPath}. {str(e)}"
        )
        sys.exit(1)
    ...
    emptyString = ""
    print(
        f"{tableHeader:^{maxTableNameLen}} type  nrows exrows  file_size rowobj_size tabobj_size get_time   t/row"
    )
    print(
        f"{emptyString:^{maxTableNameLen}}                        bytes      bytes       bytes       s        ms"
    )
    ...
    for tblName in onDemandTables:
        try:
            thisTableTime, tableSize, tableObjectSize = benchmarkTable(
                sdm, tblName, maxTableNameLen, checkRowUniqueness
            )
            ...
            print(
                f"Unexpected exception while trying to benchmark {tblName} in {sdmPath}: {str(e)}"
            )
