...
class CalPointingModelTable:
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        if not skipUniquenessCheck:
            if (
                ...
                raise DuplicateKey(
                    "Duplicate key exception in ", "CalPointingModelTable"
                )
    ...
    def toXML(self):
        ...
        result += '\n<CalPointingModelTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clpntm="http://Alma/XASDM/CalPointingModelTable" xsi:schemaLocation="http://Alma/XASDM/CalPointingModelTable http://almaobservatory.org/XML/XASDM/4/CalPointingModelTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalPointingModelTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalPointingModel"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "CalPointingModel"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("pointingModelMode")
            ...
            attributesSeq.append("azimuthRMS")

            attributesSeq.append("elevationRms")

            attributesSeq.append("skyRMS")

            attributesSeq.append("reducedChiSquared")

            attributesSeq.append("numObs")
            ...
            # c++ and Java just assume it then must be a CalPointingModel table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalPointingModelTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalPointingModel"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalPointingModel")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalPointingModelRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalPointingModelTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalPointingModel.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalPointingModel.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalPointingModel.bin")
