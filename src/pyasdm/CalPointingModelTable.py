...
class CalPointingModelTable:
    ...
    # A data structure to store the CalPointingModelRow s.
    # In all cases we maintain a private list of CalPointingModelRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  CalPointingModelRow s. Not needed in python.
    ...
    def __init__(self, container):
        """
        Create a CalPointingModelTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("CalPointingModel is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        # the single row case
        if not isinstance(x, CalPointingModelRow):
            raise ValueError("x must be a  CalPointingModelRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new CalPointingModelRow initialized to the specified values.
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of CalPointingModelRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "CalPointingModelTable"
            )
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalPointingModelTable")
        return
        ...
        # .append("\n")
        # .append("<CalPointingModelTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:clpntm=\"http://Alma/XASDM/CalPointingModelTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalPointingModelTable http://almaobservatory.org/XML/XASDM/4/CalPointingModelTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalPointingModelTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalPointingModel")
        return
        ...
        #    for (CalPointingModelRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalPointingModel");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalPointingModel");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalPointingModel");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaName"); attributesSeq.add("receiverBand"); attributesSeq.add("calDataId"); attributesSeq.add("calReductionId"); attributesSeq.add("startValidTime"); attributesSeq.add("endValidTime"); attributesSeq.add("antennaMake"); attributesSeq.add("pointingModelMode"); attributesSeq.add("polarizationType"); attributesSeq.add("numCoeff"); attributesSeq.add("coeffName"); attributesSeq.add("coeffVal"); attributesSeq.add("coeffError"); attributesSeq.add("coeffFixed"); attributesSeq.add("azimuthRMS"); attributesSeq.add("elevationRms"); attributesSeq.add("skyRMS"); attributesSeq.add("reducedChiSquared");
    #     attributesSeq.add("numObs");  attributesSeq.add("coeffFormula");
    ...
    #    else {
    #        // Then it must be a "<CalPointingModelTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalPointingModelTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalPointingModelTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalPointingModel");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalPointingModelTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalPointingModel");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalPointingModelTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalPointingModelTable/Attributes#'. Invalid XML header '"+header+"'.", "CalPointingModel");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalPointingModel");
    #    }
    #
    ...
    #    this.checkAndAdd(CalPointingModelRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalPointingModelTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalPointingModel.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalPointingModel");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalPointingModel");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        try:
            ...
            # reraise it as a ConversionException
            raise ConversionException(str(exc), "CalPointingModelTable") from None
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalPointingModel found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CalPointingModel.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CalPointingModel.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/CalPointingModel.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalPointingModel");
            #
            # File binFile = new File(directory+"/CalPointingModel.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalPointingModel");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalPointingModel");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalPointingModel");
        # }
