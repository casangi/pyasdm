...
class PolarizationTable:
    ...
    polarizationId Tag (auto-incrementable)     Polarization Table identifier </TD>
    ...
    numCorr (numCorr) int  Number of correlation products
    ...
    # A data structure to store the PolarizationRow s.
    # In all cases we maintain a private list of PolarizationRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  PolarizationRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  polarizationId to Tag(nextInt).
            x.setPolarizationId(Tag(nextInt, TagType.Polarization))
    ...
    def __init__(self, container):
        """
        Create a PolarizationTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Polarization is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        homologues in x.  If a row is found that row else autoincrement x.\polarizationId,
        add x to its table and returns x.

        returns a PolarizationRow.
        x. A row to be added.
        """
        if not isinstance(x, PolarizationRow):
            raise ValueError("x must be a  PolarizationRow instance.")

        aRow = self.lookup(x.getNumCorr(), x.getCorrType(), x.getCorrProduct())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, numCorr, corrType, corrProduct):
        """
        Create a new PolarizationRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.lookup(x.getNumCorr(), x.getCorrType(), x.getCorrProduct()) is not None:
            raise UniquenessViolationException(
                "Uniqueness violation exception in table PolarizationTable"
            )

        if self.getRowByKey(x.getPolarizationId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of PolarizationRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "PolarizationTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "PolarizationTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <PolarizationTable")
        return
        ...
        # .append("\n")
        # .append("<PolarizationTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:plrztn=\"http://Alma/XASDM/PolarizationTable\" xsi:schemaLocation=\"http://Alma/XASDM/PolarizationTable http://almaobservatory.org/XML/XASDM/4/PolarizationTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='PolarizationTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<polarizationId/>\n")
        # .append("<numCorr/>\n")
        # .append("<corrType/>\n")
        # .append("<corrProduct/>\n")

        # .append("</Attributes>\n")
        # .append("</PolarizationTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Polarization")
        return
        ...
        #    for (PolarizationRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Polarization");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Polarization");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Polarization");
    # }
    #
    # //
    ...
    #     attributesSeq.add("polarizationId"); attributesSeq.add("numCorr"); attributesSeq.add("corrType"); attributesSeq.add("corrProduct");
    #
    ...
    #    else {
    #        // Then it must be a "<PolarizationTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/PolarizationTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/PolarizationTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Polarization");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/PolarizationTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Polarization");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/PolarizationTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/PolarizationTable/Attributes#'. Invalid XML header '"+header+"'.", "Polarization");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Polarization");
    #    }
    #
    ...
    #    this.checkAndAdd(PolarizationRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for PolarizationTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Polarization.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Polarization");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Polarization");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Polarization found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Polarization.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Polarization.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/Polarization.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Polarization");
            #
            # File binFile = new File(directory+"/Polarization.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Polarization");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Polarization");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Polarization");
        # }
