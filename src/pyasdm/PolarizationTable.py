...
class PolarizationTable:
    ...
    Attributes:
        ...
        polarizationId (Tag): Polarization Table identifier auto-incrementable, key.
        ...
        corrType (StokesParameter [] ): Array(numCorr) Correlation type

        corrProduct (PolarizationType []  [] ): Array(numCorr, 2) Correlation product.
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        if not skipUniquenessCheck:
            if (
                self.lookup(x.getNumCorr(), x.getCorrType(), x.getCorrProduct())
                is not None
            ):
    ...
    def toXML(self):
        ...
        result += '\n<PolarizationTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:plrztn="http://Alma/XASDM/PolarizationTable" xsi:schemaLocation="http://Alma/XASDM/PolarizationTable http://almaobservatory.org/XML/XASDM/4/PolarizationTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "PolarizationTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='PolarizationTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Polarization"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Polarization"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Polarization")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("polarizationId")

            attributesSeq.append("numCorr")

            attributesSeq.append("corrType")

            attributesSeq.append("corrProduct")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Polarization table
            # this is more insistant, just in case
            if hdrdom.nodeName != "PolarizationTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Polarization"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Polarization")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    PolarizationRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Polarization.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Polarization.xml")
            ...
            mimeFilePath = os.path.join(directory, "Polarization.bin")
