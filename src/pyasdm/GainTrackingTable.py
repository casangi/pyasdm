...
class GainTrackingTable:
    ...
    feedId int refers to a unique row in Feed Table </TD>
    ...
    attenuator  float []   numReceptor  the nominal value of the attenuator (one value per receptor).
    ...
    samplingLevel  float  the sampling level.

    numAttFreq (numAttFreq) int  the sizes of attSpectrum and attFreq.

    attFreq  float []   numAttFreq  the attenuator frequencies.

    attSpectrum  Complex []   numAttFreq  the attenuator's measured spectrum.
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <GainTrackingTable")
        return
        ...
        # .append("\n")
        # .append("<GainTrackingTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:gntrk=\"http://Alma/XASDM/GainTrackingTable\" xsi:schemaLocation=\"http://Alma/XASDM/GainTrackingTable http://almaobservatory.org/XML/XASDM/4/GainTrackingTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='GainTrackingTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<samplingLevel/>\n")
        # .append("<numAttFreq/>\n")
        # .append("<attFreq/>\n")
        # .append("<attSpectrum/>\n")
        # .append("</Attributes>\n")
        # .append("</GainTrackingTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for GainTracking")
        return
        ...
        #    for (GainTrackingRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "GainTracking");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "GainTracking");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "GainTracking");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("timeInterval"); attributesSeq.add("feedId"); attributesSeq.add("numReceptor"); attributesSeq.add("attenuator"); attributesSeq.add("polarizationType");
    #     attributesSeq.add("samplingLevel");  attributesSeq.add("numAttFreq");  attributesSeq.add("attFreq");  attributesSeq.add("attSpectrum");
    ...
    #    else {
    #        // Then it must be a "<GainTrackingTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/GainTrackingTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/GainTrackingTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "GainTracking");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/GainTrackingTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "GainTracking");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/GainTrackingTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/GainTrackingTable/Attributes#'. Invalid XML header '"+header+"'.", "GainTracking");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "GainTracking");
    #    }
    #
    ...
    #    this.checkAndAdd(GainTrackingRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for GainTrackingTable")
        return

        # java code looks like this
        # File file = new File(directory+"/GainTracking.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "GainTracking");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "GainTracking");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "GainTracking found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for GainTracking")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/GainTracking.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "GainTracking");
            #
            # File binFile = new File(directory+"/GainTracking.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "GainTracking");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "GainTracking");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "GainTracking");
        # }
