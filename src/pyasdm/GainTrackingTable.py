...
class GainTrackingTable(Representable):
    """
    ...
    feedId int refers to a unique row in Feed Table
    ...
    numAttFreq int  the sizes of attSpectrum and attFreq.

    attFreq double []   numAttFreq  the attenuator frequencies.
    ...
    # A list to store the GainTrackingRow instances
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x is a GainTrackingRow to be inserted.
        ...
        if start.get() == rowlist[k0].timeInterval.getStart().get():
            if row.get[k0].equalByRequiredValue(x):
                ...
                # the time matches, but the rest of the required parameters do not, duplicate keys
                raise DuplicateKey("DuplicateKey exception in ", "GainTrackingTable")
        elif start.get() == rowlist[k1].timeInterval.getStart().get():
            if rowlist[k1].equalByRequiredValue(x):
                return rowlist[k1]
            else:
    ...
    def __init__(self, container):
        """
        Create a GainTrackingTable attached to container, which must be a ASDM instance
    ...
    def add(self, row):
        """
        Add a row.
        row the GainTrackingRow to be added.

        return a GainTrackingRow. If the table contains a GainTrackingRow whose attributes (key and mandatory values) are equal to those in row
        then this returns that previously added GainTrackingRow, otherwise row is returned.

        raises DuplicateKey when the table contains a GainTrackingRow with a key equal to the key in row but having
    ...
    def newRow(
        ...
        """
        Create a new GainTrackingRow. The new row is not added to this table, but it does know about it.
    ...
    def get(self):
        """
        Get all rows as a list of GainTrackingRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId, feedId):
        ...
        """
        thisKey = GainTrackingTable.Key(antennaId, spectralWindowId, feedId)
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from a the expected table", "GainTrackingTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException(
                "schemaVersion not found in XML", "GainTrackingTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "GainTrackingTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("GainTracking.xml is empty", "GainTrackingTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'GainTracking.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'GainTracking.xml'.
