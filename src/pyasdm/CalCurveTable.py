...
class CalCurveTable:
    ...
    Attributes:
        ...
        frequencyRange (Frequency [] ): Array(2) the range of frequencies over which the result is valid.
        ...
        curve (float []  []  [] ): Array(numAntenna, numReceptor, numPoly) the coefficients of the polynomials (one array of numPoly coefficients per receptor per antenna).
        ...
        rms (float []  [] ): Array(numReceptor, numBaseline) the amplitude or phase residuals (one array of numBaselines values per receptor). Optional.
    ...
    def toXML(self):
        ...
        result += '\n<CalCurveTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clcrv="http://Alma/XASDM/CalCurveTable" xsi:schemaLocation="http://Alma/XASDM/CalCurveTable http://almaobservatory.org/XML/XASDM/4/CalCurveTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalCurveTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalCurve"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalCurve"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalCurve")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            # c++ and Java just assume it then must be a CalCurve table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalCurveTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalCurve"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalCurve")
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalCurve.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalCurve.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalCurve.bin")
