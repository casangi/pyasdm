...
class SubscanTable(Representable):
    """
    ...
    execBlockId Tag identifies a unique row in ExecBlockTable.

    scanNumber int the number of the scan this subscan belongs to.

    subscanNumber int the subscan number.
    ...
    numSubintegration int []   numIntegration  the number of subintegrations for each integration.
    ...
    # A list to store the SubscanRow instances
    _privateRows = []
    ...
    @staticmethod
    def Key(execBlockId, scanNumber, subscanNumber):
        """
        ...
        result += subscanNumber() + "_"

        return result

    def __init__(self, container):
        """
        Create a SubscanTable attached to container, which must be a ASDM instance
    ...
    def add(self, newrow):
        ...
        if (
            self.getRowByKey(
                newrow.getExecBlockId(),
                newrow.getScanNumber(),
                newrow.getSubscanNumber(),
            )
            is not None
        ):
            raise DuplicateKey(
    ...
    def newRow(
        ...
        """
        Create a new SubscanRow. The new row is not added to this table, but it does know about it.
    ...
    def get(self):
        """
        Get all rows as an array of SubscanRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "SubscanTable":
            raise ConversionException(
                "XML is not from a the expected table", "SubscanTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException("schemaVersion not found in XML", "SubscanTable")
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "SubscanTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("Subscan.xml is empty", "SubscanTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'Subscan.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'Subscan.xml'.
