...
class SubscanTable:
    ...
    Attributes:
        ...
        execBlockId (Tag): identifies a unique row in ExecBlockTable. key.



        scanNumber (int): the number of the scan this subscan belongs to. key.



        subscanNumber (int): the subscan number. key.
        ...
        numIntegration (int): the number of integrations during the scan.

        numSubintegration (int [] ): Array(numIntegration) the number of subintegrations for each integration.




        subscanMode (SwitchingMode): identifies the data acquisition mode during the subscan. Optional.

        correlatorCalibration (CorrelatorCalibration): identifies the correlator calibration during the subscan. Optional.
    ...
    def toXML(self):
        ...
        result += '\n<SubscanTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sbscn="http://Alma/XASDM/SubscanTable" xsi:schemaLocation="http://Alma/XASDM/SubscanTable http://almaobservatory.org/XML/XASDM/4/SubscanTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='SubscanTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<endTime/>\n"
        result += "<fieldName/>\n"
        result += "<subscanIntent/>\n"
        result += "<numIntegration/>\n"
        result += "<numSubintegration/>\n"

        result += "<subscanMode/>\n"
        result += "<correlatorCalibration/>\n"
        result += "</Attributes>\n"
        result += "</SubscanTable>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Subscan"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Subscan"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Subscan")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("subscanIntent")

            attributesSeq.append("numIntegration")

            attributesSeq.append("numSubintegration")

            attributesSeq.append("subscanMode")

            attributesSeq.append("correlatorCalibration")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Subscan table
            # this is more insistant, just in case
            if hdrdom.nodeName != "SubscanTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Subscan"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Subscan"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Subscan")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    SubscanRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "SubscanTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Subscan.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Subscan.xml")
            ...
            mimeFilePath = os.path.join(directory, "Subscan.bin")
