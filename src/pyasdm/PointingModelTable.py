...
class PointingModelTable:
    ...
    def toXML(self):
        ...
        result += '\n<PointingModelTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:pointm="http://Alma/XASDM/PointingModelTable" xsi:schemaLocation="http://Alma/XASDM/PointingModelTable http://almaobservatory.org/XML/XASDM/4/PointingModelTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "PointingModelTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='PointingModelTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "PointingModel"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "PointingModel"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "PointingModel"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("pointingModelId")

            attributesSeq.append("numCoeff")

            attributesSeq.append("coeffName")

            attributesSeq.append("coeffVal")
            ...
            attributesSeq.append("assocPointingModelId")
            ...
            # c++ and Java just assume it then must be a PointingModel table
            # this is more insistant, just in case
            if hdrdom.nodeName != "PointingModelTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "PointingModel"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("PointingModel")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    PointingModelRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "PointingModel.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "PointingModel.xml")
            ...
            mimeFilePath = os.path.join(directory, "PointingModel.bin")
