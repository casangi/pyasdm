...
class PointingModelTable:
    ...
    # A data structure to store the PointingModelRow s.
    # In all cases we maintain a private list of PointingModelRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  PointingModelRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  pointingModelId to nextInt.
            x.setPointingModelId(nextInt)
    ...
    def __init__(self, container):
        """
        Create a PointingModelTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("PointingModel is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, PointingModelRow):
            raise ValueError("x must be a  PointingModelRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new PointingModelRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getAntennaId(), x.getPointingModelId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of PointingModelRow
        """
    ...
    def getRowByPointingModelId(pointingModelId):
        """
        ...
        param pointingModelId of type int contains the value of
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "PointingModelTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "PointingModelTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <PointingModelTable")
        return
        ...
        # .append("\n")
        # .append("<PointingModelTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:pointm=\"http://Alma/XASDM/PointingModelTable\" xsi:schemaLocation=\"http://Alma/XASDM/PointingModelTable http://almaobservatory.org/XML/XASDM/4/PointingModelTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='PointingModelTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<receiverBand/>\n")
        # .append("<assocNature/>\n")
        # .append("<assocPointingModelId/>\n")

        # .append("<coeffFormula/>\n")
        # .append("</Attributes>\n")
        # .append("</PointingModelTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for PointingModel")
        return
        ...
        #    for (PointingModelRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "PointingModel");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "PointingModel");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "PointingModel");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("pointingModelId"); attributesSeq.add("numCoeff"); attributesSeq.add("coeffName"); attributesSeq.add("coeffVal"); attributesSeq.add("polarizationType"); attributesSeq.add("receiverBand"); attributesSeq.add("assocNature"); attributesSeq.add("assocPointingModelId");
    #     attributesSeq.add("coeffFormula");
    ...
    #    else {
    #        // Then it must be a "<PointingModelTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/PointingModelTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/PointingModelTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "PointingModel");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/PointingModelTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "PointingModel");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/PointingModelTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/PointingModelTable/Attributes#'. Invalid XML header '"+header+"'.", "PointingModel");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "PointingModel");
    #    }
    #
    ...
    #    this.checkAndAdd(PointingModelRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for PointingModelTable")
        return

        # java code looks like this
        # File file = new File(directory+"/PointingModel.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "PointingModel");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "PointingModel");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "PointingModel found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "PointingModel.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "PointingModel.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/PointingModel.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "PointingModel");
            #
            # File binFile = new File(directory+"/PointingModel.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "PointingModel");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "PointingModel");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "PointingModel");
        # }
