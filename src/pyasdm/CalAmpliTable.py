...
class CalAmpliTable:
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalAmpliTable")
        return
        ...
        # .append("\n")
        # .append("<CalAmpliTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:clmpl=\"http://Alma/XASDM/CalAmpliTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalAmpliTable http://almaobservatory.org/XML/XASDM/4/CalAmpliTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalAmpliTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<apertureEfficiency/>\n")
        # .append("<apertureEfficiencyError/>\n")

        # .append("<correctionValidity/>\n")
        # .append("</Attributes>\n")
        # .append("</CalAmpliTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalAmpli")
        return
        ...
        #    for (CalAmpliRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalAmpli");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalAmpli");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalAmpli");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaName"); attributesSeq.add("atmPhaseCorrection"); attributesSeq.add("receiverBand"); attributesSeq.add("basebandName"); attributesSeq.add("calDataId"); attributesSeq.add("calReductionId"); attributesSeq.add("numReceptor"); attributesSeq.add("polarizationTypes"); attributesSeq.add("startValidTime"); attributesSeq.add("endValidTime"); attributesSeq.add("frequencyRange"); attributesSeq.add("apertureEfficiency"); attributesSeq.add("apertureEfficiencyError");
    #     attributesSeq.add("correctionValidity");
    ...
    #    else {
    #        // Then it must be a "<CalAmpliTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalAmpliTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalAmpliTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalAmpli");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalAmpliTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalAmpli");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalAmpliTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalAmpliTable/Attributes#'. Invalid XML header '"+header+"'.", "CalAmpli");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalAmpli");
    #    }
    #
    ...
    #    this.checkAndAdd(CalAmpliRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalAmpliTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalAmpli.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalAmpli");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalAmpli");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalAmpli found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/CalAmpli.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalAmpli");
            #
            # File binFile = new File(directory+"/CalAmpli.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalAmpli");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalAmpli");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalAmpli");
        # }
