...
class CalFocusModelTable:
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalFocusModelTable")
        return
        ...
        # .append("\n")
        # .append("<CalFocusModelTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:clfcsm=\"http://Alma/XASDM/CalFocusModelTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalFocusModelTable http://almaobservatory.org/XML/XASDM/4/CalFocusModelTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalFocusModelTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalFocusModel")
        return
        ...
        #    for (CalFocusModelRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalFocusModel");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalFocusModel");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalFocusModel");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaName"); attributesSeq.add("receiverBand"); attributesSeq.add("polarizationType"); attributesSeq.add("calDataId"); attributesSeq.add("calReductionId"); attributesSeq.add("startValidTime"); attributesSeq.add("endValidTime"); attributesSeq.add("antennaMake"); attributesSeq.add("numCoeff"); attributesSeq.add("numSourceObs"); attributesSeq.add("coeffName"); attributesSeq.add("coeffFormula"); attributesSeq.add("coeffValue"); attributesSeq.add("coeffError"); attributesSeq.add("coeffFixed"); attributesSeq.add("focusModel"); attributesSeq.add("focusRMS"); attributesSeq.add("reducedChiSquared");
    #
    ...
    #    else {
    #        // Then it must be a "<CalFocusModelTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalFocusModelTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalFocusModelTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalFocusModel");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalFocusModelTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalFocusModel");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalFocusModelTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalFocusModelTable/Attributes#'. Invalid XML header '"+header+"'.", "CalFocusModel");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalFocusModel");
    #    }
    #
    ...
    #    this.checkAndAdd(CalFocusModelRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalFocusModelTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalFocusModel.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalFocusModel");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalFocusModel");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalFocusModel found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for CalFocusModel")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/CalFocusModel.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalFocusModel");
            #
            # File binFile = new File(directory+"/CalFocusModel.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalFocusModel");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalFocusModel");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalFocusModel");
        # }
