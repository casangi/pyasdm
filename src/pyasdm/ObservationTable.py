...
class ObservationTable:
    ...
    Attributes:
        ...
        observationId (Tag):  auto-incrementable, key.
    ...
    def toXML(self):
        ...
        result += '\n<ObservationTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:obsrvn="http://Alma/XASDM/ObservationTable" xsi:schemaLocation="http://Alma/XASDM/ObservationTable http://almaobservatory.org/XML/XASDM/4/ObservationTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "ObservationTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='ObservationTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Observation"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Observation"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Observation")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("observationId")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Observation table
            # this is more insistant, just in case
            if hdrdom.nodeName != "ObservationTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Observation"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Observation")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    ObservationRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Observation.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Observation.xml")
            ...
            mimeFilePath = os.path.join(directory, "Observation.bin")
