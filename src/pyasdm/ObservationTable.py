...
class ObservationTable:
    ...
    observationId Tag (auto-incrementable)      </TD>
    ...
    # A data structure to store the ObservationRow s.
    # In all cases we maintain a private list of ObservationRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  ObservationRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  observationId to Tag(nextInt).
            x.setObservationId(Tag(nextInt, TagType.Observation))
    ...
    def __init__(self, container):
        """
        Create a ObservationTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("ObservationTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("ObservationTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Observation is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "ObservationTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = ObservationRow(self)
        return thisRow

    def add(self, x):
        """
        Look up the table for a row whose noautoincrementable attributes are matching their
        homologues in x.  If a row is found that row else autoincrement x.\observationId,
        add x to its table and returns x.

        returns a ObservationRow.
        x. A row to be added.
        """
        if not isinstance(x, ObservationRow):
            raise ValueError("x must be a  ObservationRow instance.")
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new ObservationRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new ObservationRow with default values for its attributes.
        """

        return ObservationRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getObservationId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "ObservationTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of ObservationRow
        """
        return self._privateRows

    def getRowByKey(self, observationId):
        """
        ...
        for row in self._privateRows:
            ...
            if not row.getObservationId().equals(observationId):
                continue
    ...
    def toXML(self):
        ...
        result += '<ObservationTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:obsrvn="http://Alma/XASDM/ObservationTable" xsi:schemaLocation="http://Alma/XASDM/ObservationTable http://almaobservatory.org/XML/XASDM/4/ObservationTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a Observation (ObservationTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "ObservationTable".
        if (
            not xmldom.hasChildNodes()
            or xmldom.firstChild.nodeName != "ObservationTable"
        ):
            raise ConversionException(
                "XML is not from the expected table", "ObservationTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "ObservationTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "ObservationTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "ObservationTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "ObservationTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "ObservationTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "ObservationTable") from None

                except UniquenessViolationException as exc:
                    msg = (
                        "UniquenessViolationException in row in ObservationTable : %s"
                        % str(exc)
                    )

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "ObservationTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "ObservationTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <ObservationTable")
        return
        ...
        # .append("\n")
        # .append("<ObservationTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:obsrvn=\"http://Alma/XASDM/ObservationTable\" xsi:schemaLocation=\"http://Alma/XASDM/ObservationTable http://almaobservatory.org/XML/XASDM/4/ObservationTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='ObservationTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Observation")
        return
        ...
        #    for (ObservationRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Observation");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Observation");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Observation");
    # }
    #
    # //
    ...
    #     attributesSeq.add("observationId");
    #
    ...
    #    else {
    #        // Then it must be a "<ObservationTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/ObservationTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/ObservationTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Observation");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/ObservationTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Observation");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/ObservationTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/ObservationTable/Attributes#'. Invalid XML header '"+header+"'.", "Observation");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Observation");
    #    }
    #
    ...
    #    this.checkAndAdd(ObservationRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "Observation.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "Observation.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the Observation table", "ObservationTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for ObservationTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Observation.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Observation");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Observation");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "Observation.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Observation found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Observation.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Observation.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (Observation)"
                % directory,
                "ObservationTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for Observation")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/Observation.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Observation");
            #
            # File binFile = new File(directory+"/Observation.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Observation");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Observation");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Observation");
        # }
        # }
            ...
            filePath = os.path.join(directory, "Observation.xml")
