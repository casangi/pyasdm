...
class FlagCmdTable:
    ...
    Attributes:
        ...
        timeInterval (ArrayTimeInterval):  key.
    ...
    def toXML(self):
        ...
        result += '\n<FlagCmdTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:flgcmd="http://Alma/XASDM/FlagCmdTable" xsi:schemaLocation="http://Alma/XASDM/FlagCmdTable http://almaobservatory.org/XML/XASDM/4/FlagCmdTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='FlagCmdTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "FlagCmd"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "FlagCmd"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "FlagCmd")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("level")

            attributesSeq.append("severity")

            attributesSeq.append("applied")

            attributesSeq.append("command")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a FlagCmd table
            # this is more insistant, just in case
            if hdrdom.nodeName != "FlagCmdTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "FlagCmd"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "FlagCmd"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("FlagCmd")
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "FlagCmd.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "FlagCmd.xml")
            ...
            mimeFilePath = os.path.join(directory, "FlagCmd.bin")
