...
class FlagCmdTable:
    ...
    # A data structure to store the FlagCmdRow s.
    # In all cases we maintain a private list of FlagCmdRow s.
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x The FlagCmdRow to be inserted.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("FlagCmd is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        """
        if not isinstance(x, FlagCmdRow):
            raise ValueError("x must be a  FlagCmdRow instance.")
    ...
    def newRow(self, timeInterval, type, reason, level, severity, applied, command):
        """
        Create a new FlagCmdRow initialized to the specified values.
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of FlagCmdRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "FlagCmdTable":
            raise ConversionException(
                "XML is not from the expected table", "FlagCmdTable"
            )
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <FlagCmdTable")
        return
        ...
        # .append("\n")
        # .append("<FlagCmdTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:flgcmd=\"http://Alma/XASDM/FlagCmdTable\" xsi:schemaLocation=\"http://Alma/XASDM/FlagCmdTable http://almaobservatory.org/XML/XASDM/4/FlagCmdTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='FlagCmdTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for FlagCmd")
        return
        ...
        #    for (FlagCmdRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "FlagCmd");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "FlagCmd");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "FlagCmd");
    # }
    #
    # //
    ...
    #     attributesSeq.add("timeInterval"); attributesSeq.add("type"); attributesSeq.add("reason"); attributesSeq.add("level"); attributesSeq.add("severity"); attributesSeq.add("applied"); attributesSeq.add("command");
    #
    ...
    #    else {
    #        // Then it must be a "<FlagCmdTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/FlagCmdTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/FlagCmdTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "FlagCmd");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/FlagCmdTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "FlagCmd");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/FlagCmdTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/FlagCmdTable/Attributes#'. Invalid XML header '"+header+"'.", "FlagCmd");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "FlagCmd");
    #    }
    #
    ...
    #    this.checkAndAdd(FlagCmdRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for FlagCmdTable")
        return

        # java code looks like this
        # File file = new File(directory+"/FlagCmd.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "FlagCmd");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "FlagCmd");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "FlagCmd found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "FlagCmd.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "FlagCmd.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/FlagCmd.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "FlagCmd");
            #
            # File binFile = new File(directory+"/FlagCmd.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "FlagCmd");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "FlagCmd");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "FlagCmd");
        # }
