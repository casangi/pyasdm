...
class CalPrimaryBeamTable:
    ...
    # A data structure to store the CalPrimaryBeamRow s.
    # In all cases we maintain a private list of CalPrimaryBeamRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  CalPrimaryBeamRow s. Not needed in python.
    ...
    def __init__(self, container):
        """
        Create a CalPrimaryBeamTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("CalPrimaryBeam is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        # the single row case
        if not isinstance(x, CalPrimaryBeamRow):
            raise ValueError("x must be a  CalPrimaryBeamRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new CalPrimaryBeamRow initialized to the specified values.
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of CalPrimaryBeamRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "CalPrimaryBeamTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "CalPrimaryBeamTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalPrimaryBeamTable")
        return
        ...
        # .append("\n")
        # .append("<CalPrimaryBeamTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:clprbm=\"http://Alma/XASDM/CalPrimaryBeamTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalPrimaryBeamTable http://almaobservatory.org/XML/XASDM/4/CalPrimaryBeamTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalPrimaryBeamTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<mainBeamEfficiency/>\n")
        # .append("<beamDescriptionUID/>\n")
        # .append("<relativeAmplitudeRms/>\n")
        # .append("<direction/>\n")
        # .append("<minValidDirection/>\n")
        # .append("<maxValidDirection/>\n")
        # .append("<descriptionType/>\n")
        # .append("<imageChannelNumber/>\n")
        # .append("<imageNominalFrequency/>\n")

        # .append("</Attributes>\n")
        # .append("</CalPrimaryBeamTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalPrimaryBeam")
        return
        ...
        #    for (CalPrimaryBeamRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalPrimaryBeam");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalPrimaryBeam");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalPrimaryBeam");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaName"); attributesSeq.add("receiverBand"); attributesSeq.add("calDataId"); attributesSeq.add("calReductionId"); attributesSeq.add("startValidTime"); attributesSeq.add("endValidTime"); attributesSeq.add("antennaMake"); attributesSeq.add("numSubband"); attributesSeq.add("frequencyRange"); attributesSeq.add("numReceptor"); attributesSeq.add("polarizationTypes"); attributesSeq.add("mainBeamEfficiency"); attributesSeq.add("beamDescriptionUID"); attributesSeq.add("relativeAmplitudeRms"); attributesSeq.add("direction"); attributesSeq.add("minValidDirection"); attributesSeq.add("maxValidDirection"); attributesSeq.add("descriptionType"); attributesSeq.add("imageChannelNumber"); attributesSeq.add("imageNominalFrequency");
    #
    ...
    #    else {
    #        // Then it must be a "<CalPrimaryBeamTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalPrimaryBeamTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalPrimaryBeamTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalPrimaryBeam");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalPrimaryBeamTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalPrimaryBeam");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalPrimaryBeamTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalPrimaryBeamTable/Attributes#'. Invalid XML header '"+header+"'.", "CalPrimaryBeam");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalPrimaryBeam");
    #    }
    #
    ...
    #    this.checkAndAdd(CalPrimaryBeamRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalPrimaryBeamTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalPrimaryBeam.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalPrimaryBeam");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalPrimaryBeam");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalPrimaryBeam found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CalPrimaryBeam.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CalPrimaryBeam.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/CalPrimaryBeam.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalPrimaryBeam");
            #
            # File binFile = new File(directory+"/CalPrimaryBeam.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalPrimaryBeam");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalPrimaryBeam");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalPrimaryBeam");
        # }
