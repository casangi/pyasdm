...
class CalPrimaryBeamTable:
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalPrimaryBeamTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<mainBeamEfficiency/>\n"
        result += "<beamDescriptionUID/>\n"
        result += "<relativeAmplitudeRms/>\n"
        result += "<direction/>\n"
        result += "<minValidDirection/>\n"
        result += "<maxValidDirection/>\n"
        result += "<descriptionType/>\n"
        result += "<imageChannelNumber/>\n"
        result += "<imageNominalFrequency/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalPrimaryBeam"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalPrimaryBeam"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "CalPrimaryBeam"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numSubband")
            ...
            attributesSeq.append("mainBeamEfficiency")

            attributesSeq.append("beamDescriptionUID")

            attributesSeq.append("relativeAmplitudeRms")

            attributesSeq.append("direction")

            attributesSeq.append("minValidDirection")

            attributesSeq.append("maxValidDirection")

            attributesSeq.append("descriptionType")

            attributesSeq.append("imageChannelNumber")

            attributesSeq.append("imageNominalFrequency")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalPrimaryBeam table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalPrimaryBeamTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalPrimaryBeam"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalPrimaryBeam")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalPrimaryBeamRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalPrimaryBeamTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalPrimaryBeam.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalPrimaryBeam.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalPrimaryBeam.bin")
