...
class FieldTable:
    ...
    def toXML(self):
        ...
        result += '\n<FieldTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:field="http://Alma/XASDM/FieldTable" xsi:schemaLocation="http://Alma/XASDM/FieldTable http://almaobservatory.org/XML/XASDM/4/FieldTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "FieldTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='FieldTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Field"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Field"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Field")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("delayDir")

            attributesSeq.append("phaseDir")

            attributesSeq.append("referenceDir")
            ...
            attributesSeq.append("assocFieldId")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Field table
            # this is more insistant, just in case
            if hdrdom.nodeName != "FieldTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Field"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Field"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Field")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    FieldRow.fromBin(eis, self, attributesSeq), self._checkRowUniqueness
                )
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Field.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Field.xml")
            ...
            mimeFilePath = os.path.join(directory, "Field.bin")
