...
class SBSummaryTable:
    ...
    def toXML(self):
        ...
        result += '\n<SBSummaryTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sbsmmr="http://Alma/XASDM/SBSummaryTable" xsi:schemaLocation="http://Alma/XASDM/SBSummaryTable http://almaobservatory.org/XML/XASDM/4/SBSummaryTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "SBSummaryTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='SBSummaryTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<sbSummaryUID/>\n"
        result += "<projectUID/>\n"
        result += "<obsUnitSetUID/>\n"
        result += "<frequency/>\n"
        result += "<frequencyBand/>\n"
        result += "<sbType/>\n"
        result += "<sbDuration/>\n"
        result += "<numObservingMode/>\n"
        result += "<observingMode/>\n"
        result += "<numberRepeats/>\n"
        result += "<numScienceGoal/>\n"
        result += "<scienceGoal/>\n"
        result += "<numWeatherConstraint/>\n"
        result += "<weatherConstraint/>\n"

        result += "<centerDirection/>\n"
        result += "<centerDirectionCode/>\n"
        result += "<centerDirectionEquinox/>\n"
        result += "</Attributes>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "SBSummary"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "SBSummary"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "SBSummary")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("sbSummaryUID")

            attributesSeq.append("projectUID")

            attributesSeq.append("obsUnitSetUID")

            attributesSeq.append("frequency")

            attributesSeq.append("frequencyBand")

            attributesSeq.append("sbType")

            attributesSeq.append("sbDuration")

            attributesSeq.append("numObservingMode")

            attributesSeq.append("observingMode")

            attributesSeq.append("numberRepeats")

            attributesSeq.append("numScienceGoal")

            attributesSeq.append("scienceGoal")

            attributesSeq.append("numWeatherConstraint")

            attributesSeq.append("weatherConstraint")

            attributesSeq.append("centerDirection")

            attributesSeq.append("centerDirectionCode")

            attributesSeq.append("centerDirectionEquinox")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a SBSummary table
            # this is more insistant, just in case
            if hdrdom.nodeName != "SBSummaryTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "SBSummary"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("SBSummary")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    SBSummaryRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "SBSummary.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "SBSummary.xml")
            ...
            mimeFilePath = os.path.join(directory, "SBSummary.bin")
