...
class SBSummaryTable:
    ...
    # A data structure to store the SBSummaryRow s.
    # In all cases we maintain a private list of SBSummaryRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  SBSummaryRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  sBSummaryId to Tag(nextInt).
            x.setSBSummaryId(Tag(nextInt, TagType.SBSummary))
    ...
    def __init__(self, container):
        """
        Create a SBSummaryTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("SBSummary is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, SBSummaryRow):
            raise ValueError("x must be a  SBSummaryRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new SBSummaryRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getSBSummaryId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of SBSummaryRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "SBSummaryTable":
            raise ConversionException(
                "XML is not from the expected table", "SBSummaryTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "SBSummaryTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <SBSummaryTable")
        return
        ...
        # .append("\n")
        # .append("<SBSummaryTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:sbsmmr=\"http://Alma/XASDM/SBSummaryTable\" xsi:schemaLocation=\"http://Alma/XASDM/SBSummaryTable http://almaobservatory.org/XML/XASDM/4/SBSummaryTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='SBSummaryTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<sbSummaryUID/>\n")
        # .append("<projectUID/>\n")
        # .append("<obsUnitSetUID/>\n")
        # .append("<frequency/>\n")
        # .append("<frequencyBand/>\n")
        # .append("<sbType/>\n")
        # .append("<sbDuration/>\n")
        # .append("<numObservingMode/>\n")
        # .append("<observingMode/>\n")
        # .append("<numberRepeats/>\n")
        # .append("<numScienceGoal/>\n")
        # .append("<scienceGoal/>\n")
        # .append("<numWeatherConstraint/>\n")
        # .append("<weatherConstraint/>\n")

        # .append("<centerDirection/>\n")
        # .append("<centerDirectionCode/>\n")
        # .append("<centerDirectionEquinox/>\n")
        # .append("</Attributes>\n")
        # .append("</SBSummaryTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for SBSummary")
        return
        ...
        #    for (SBSummaryRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "SBSummary");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "SBSummary");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "SBSummary");
    # }
    #
    # //
    ...
    #     attributesSeq.add("sBSummaryId"); attributesSeq.add("sbSummaryUID"); attributesSeq.add("projectUID"); attributesSeq.add("obsUnitSetUID"); attributesSeq.add("frequency"); attributesSeq.add("frequencyBand"); attributesSeq.add("sbType"); attributesSeq.add("sbDuration"); attributesSeq.add("numObservingMode"); attributesSeq.add("observingMode"); attributesSeq.add("numberRepeats"); attributesSeq.add("numScienceGoal"); attributesSeq.add("scienceGoal"); attributesSeq.add("numWeatherConstraint"); attributesSeq.add("weatherConstraint");
    #     attributesSeq.add("centerDirection");  attributesSeq.add("centerDirectionCode");  attributesSeq.add("centerDirectionEquinox");
    ...
    #    else {
    #        // Then it must be a "<SBSummaryTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/SBSummaryTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/SBSummaryTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SBSummary");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/SBSummaryTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SBSummary");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/SBSummaryTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/SBSummaryTable/Attributes#'. Invalid XML header '"+header+"'.", "SBSummary");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "SBSummary");
    #    }
    #
    ...
    #    this.checkAndAdd(SBSummaryRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for SBSummaryTable")
        return

        # java code looks like this
        # File file = new File(directory+"/SBSummary.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "SBSummary");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "SBSummary");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "SBSummary found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "SBSummary.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "SBSummary.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/SBSummary.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "SBSummary");
            #
            # File binFile = new File(directory+"/SBSummary.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "SBSummary");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SBSummary");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SBSummary");
        # }
