...
class CalAtmosphereTable:
    ...
    def toXML(self):
        ...
        result += '\n<CalAtmosphereTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clatm="http://Alma/XASDM/CalAtmosphereTable" xsi:schemaLocation="http://Alma/XASDM/CalAtmosphereTable http://almaobservatory.org/XML/XASDM/4/CalAtmosphereTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalAtmosphereTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<numLoad/>\n"
        result += "<numReceptor/>\n"
        result += "<forwardEffSpectrum/>\n"
        result += "<frequencyRange/>\n"
        result += "<groundPressure/>\n"
        result += "<groundRelHumidity/>\n"
        result += "<frequencySpectrum/>\n"
        result += "<groundTemperature/>\n"
        result += "<polarizationTypes/>\n"
        result += "<powerSkySpectrum/>\n"
        result += "<powerLoadSpectrum/>\n"
        result += "<syscalType/>\n"
        result += "<tAtmSpectrum/>\n"
        result += "<tRecSpectrum/>\n"
        result += "<tSysSpectrum/>\n"
        ...
        result += "<alphaSpectrum/>\n"
        result += "<forwardEfficiency/>\n"
        result += "<forwardEfficiencyError/>\n"
        result += "<sbGain/>\n"
        result += "<sbGainError/>\n"
        result += "<sbGainSpectrum/>\n"
        result += "</Attributes>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalAtmosphere"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalAtmosphere"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "CalAtmosphere"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numLoad")

            attributesSeq.append("numReceptor")

            attributesSeq.append("forwardEffSpectrum")

            attributesSeq.append("frequencyRange")

            attributesSeq.append("groundPressure")

            attributesSeq.append("groundRelHumidity")

            attributesSeq.append("frequencySpectrum")

            attributesSeq.append("groundTemperature")

            attributesSeq.append("polarizationTypes")

            attributesSeq.append("powerSkySpectrum")

            attributesSeq.append("powerLoadSpectrum")

            attributesSeq.append("syscalType")

            attributesSeq.append("tAtmSpectrum")

            attributesSeq.append("tRecSpectrum")

            attributesSeq.append("tSysSpectrum")

            attributesSeq.append("tauSpectrum")

            attributesSeq.append("tAtm")

            attributesSeq.append("tRec")

            attributesSeq.append("tSys")

            attributesSeq.append("tau")

            attributesSeq.append("water")

            attributesSeq.append("waterError")

            attributesSeq.append("alphaSpectrum")

            attributesSeq.append("forwardEfficiency")

            attributesSeq.append("forwardEfficiencyError")

            attributesSeq.append("sbGain")

            attributesSeq.append("sbGainError")

            attributesSeq.append("sbGainSpectrum")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalAtmosphere table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalAtmosphereTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalAtmosphere"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalAtmosphere")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalAtmosphereRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalAtmosphereTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalAtmosphere.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalAtmosphere.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalAtmosphere.bin")
