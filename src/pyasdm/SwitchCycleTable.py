...
class SwitchCycleTable:
    ...
    def toXML(self):
        ...
        result += '\n<SwitchCycleTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:swtccl="http://Alma/XASDM/SwitchCycleTable" xsi:schemaLocation="http://Alma/XASDM/SwitchCycleTable http://almaobservatory.org/XML/XASDM/4/SwitchCycleTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "SwitchCycleTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='SwitchCycleTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<numStep/>\n"
        result += "<weightArray/>\n"
        result += "<dirOffsetArray/>\n"
        result += "<freqOffsetArray/>\n"
        result += "<stepDurationArray/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "SwitchCycle"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "SwitchCycle"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "SwitchCycle")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numStep")

            attributesSeq.append("weightArray")

            attributesSeq.append("dirOffsetArray")

            attributesSeq.append("freqOffsetArray")

            attributesSeq.append("stepDurationArray")
            ...
            # c++ and Java just assume it then must be a SwitchCycle table
            # this is more insistant, just in case
            if hdrdom.nodeName != "SwitchCycleTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "SwitchCycle"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("SwitchCycle")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    SwitchCycleRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "SwitchCycle.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "SwitchCycle.xml")
            ...
            mimeFilePath = os.path.join(directory, "SwitchCycle.bin")
