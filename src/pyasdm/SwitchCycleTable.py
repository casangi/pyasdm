...
class SwitchCycleTable:
    ...
    # A data structure to store the SwitchCycleRow s.
    # In all cases we maintain a private list of SwitchCycleRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  SwitchCycleRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  switchCycleId to Tag(nextInt).
            x.setSwitchCycleId(Tag(nextInt, TagType.SwitchCycle))
    ...
    def __init__(self, container):
        """
        Create a SwitchCycleTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("SwitchCycle is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, SwitchCycleRow):
            raise ValueError("x must be a  SwitchCycleRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new SwitchCycleRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getSwitchCycleId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of SwitchCycleRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "SwitchCycleTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "SwitchCycleTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <SwitchCycleTable")
        return
        ...
        # .append("\n")
        # .append("<SwitchCycleTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:swtccl=\"http://Alma/XASDM/SwitchCycleTable\" xsi:schemaLocation=\"http://Alma/XASDM/SwitchCycleTable http://almaobservatory.org/XML/XASDM/4/SwitchCycleTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='SwitchCycleTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<numStep/>\n")
        # .append("<weightArray/>\n")
        # .append("<dirOffsetArray/>\n")
        # .append("<freqOffsetArray/>\n")
        # .append("<stepDurationArray/>\n")

        # .append("<directionCode/>\n")
        # .append("<directionEquinox/>\n")
        # .append("</Attributes>\n")
        # .append("</SwitchCycleTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for SwitchCycle")
        return
        ...
        #    for (SwitchCycleRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "SwitchCycle");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "SwitchCycle");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "SwitchCycle");
    # }
    #
    # //
    ...
    #     attributesSeq.add("switchCycleId"); attributesSeq.add("numStep"); attributesSeq.add("weightArray"); attributesSeq.add("dirOffsetArray"); attributesSeq.add("freqOffsetArray"); attributesSeq.add("stepDurationArray");
    #     attributesSeq.add("directionCode");  attributesSeq.add("directionEquinox");
    ...
    #    else {
    #        // Then it must be a "<SwitchCycleTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/SwitchCycleTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/SwitchCycleTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SwitchCycle");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/SwitchCycleTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SwitchCycle");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/SwitchCycleTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/SwitchCycleTable/Attributes#'. Invalid XML header '"+header+"'.", "SwitchCycle");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "SwitchCycle");
    #    }
    #
    ...
    #    this.checkAndAdd(SwitchCycleRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for SwitchCycleTable")
        return

        # java code looks like this
        # File file = new File(directory+"/SwitchCycle.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "SwitchCycle");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "SwitchCycle");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "SwitchCycle found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "SwitchCycle.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "SwitchCycle.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/SwitchCycle.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "SwitchCycle");
            #
            # File binFile = new File(directory+"/SwitchCycle.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "SwitchCycle");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SwitchCycle");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SwitchCycle");
        # }
