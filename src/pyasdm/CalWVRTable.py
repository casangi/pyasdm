...
class CalWVRTable:
    ...
    Attributes:
        ...
        calReductionId (Tag): refers to unique row  in CalReductionTable. key.
        ...
        inputAntennaNames (str [] ): Array(numInputAntennas) the names of the input antennas (one string per antenna).

        numChan (int): the number of frequency channels in the WVR receiver.

        chanFreq (Frequency [] ): Array(numChan) the channel frequencies (one value per channel).

        chanWidth (Frequency [] ): Array(numChan) the widths of the channels (one value per channel).

        refTemp (Temperature []  [] ): Array(numInputAntennas, numChan) the reference temperatures (one value per input antenna per channel).

        numPoly (int): the number of polynomial coefficients.

        pathCoeff (float []  []  [] ): Array(numInputAntennas, numChan, numPoly) the path length coefficients (one value per input antenna per channel per polynomial coefficient).

        polyFreqLimits (Frequency [] ): Array(2) the limits of the interval of frequencies for which the path length coefficients are computed.

        wetPath (float [] ): Array(numPoly) The wet path as a function frequency (expressed as a polynomial).

        dryPath (float [] ): Array(numPoly) The dry path as a function frequency (expressed as a polynomial).
        ...
        tauBaseline (float): Constant opacity term (frequency independent). Optional.
    ...
    def toXML(self):
        ...
        result += '\n<CalWVRTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clwvr="http://Alma/XASDM/CalWVRTable" xsi:schemaLocation="http://Alma/XASDM/CalWVRTable http://almaobservatory.org/XML/XASDM/4/CalWVRTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalWVRTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<endValidTime/>\n"
        result += "<wvrMethod/>\n"
        result += "<numInputAntennas/>\n"
        result += "<inputAntennaNames/>\n"
        result += "<numChan/>\n"
        result += "<chanFreq/>\n"
        result += "<chanWidth/>\n"
        result += "<refTemp/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalWVR"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalWVR"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalWVR")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numInputAntennas")

            attributesSeq.append("inputAntennaNames")

            attributesSeq.append("numChan")

            attributesSeq.append("chanFreq")
            ...
            attributesSeq.append("wetPath")

            attributesSeq.append("dryPath")

            attributesSeq.append("water")

            attributesSeq.append("tauBaseline")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalWVR table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalWVRTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalWVR"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "CalWVR"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalWVR")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalWVRRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalWVRTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalWVR.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalWVR.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalWVR.bin")
