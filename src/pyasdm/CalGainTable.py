...
class CalGainTable:
    ...
    # A data structure to store the CalGainRow s.
    # In all cases we maintain a private list of CalGainRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  CalGainRow s. Not needed in python.
    ...
    def Key(self, calDataId, calReductionId):
        """
    ...
    def __init__(self, container):
        """
        Create a CalGainTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("CalGainTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("CalGainTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("CalGain is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "CalGainTable" followed by the current size of the table
        between parenthesis.
    ...
    def add(self, x):
        ...
        # the single row case
        if not isinstance(x, CalGainRow):
            raise ValueError("x must be a  CalGainRow instance.")

        if self.getRowByKey(x.getCalDataId(), x.getCalReductionId()) is not None:
            raise DuplicateKey(
                "[" + x.getCalDataId() + "|" + x.getCalReductionId() + "]", "CalGain"
            )
    ...
    def newRow(
        ...
        """
        Create a new CalGainRow initialized to the specified values.
        ...
        thisRow.setGain(gain)

        thisRow.setGainValid(gainValid)

        thisRow.setFit(fit)

        thisRow.setFitWeight(fitWeight)

        thisRow.setTotalGainValid(totalGainValid)

        thisRow.setTotalFit(totalFit)

        thisRow.setTotalFitWeight(totalFitWeight)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new CalGainRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new CalGainRow with default values for its attributes.
        """

        return CalGainRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getCalDataId(), x.getCalReductionId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "CalGainTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of CalGainRow
        """
        return self._privateRows

    def getRowByKey(self, calDataId, calReductionId):
        """
    ...
    def toXML(self):
        ...
        result += '<CalGainTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clgn="http://Alma/XASDM/CalGainTable" xsi:schemaLocation="http://Alma/XASDM/CalGainTable http://almaobservatory.org/XML/XASDM/4/CalGainTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a CalGain (CalGainTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "CalGainTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "CalGainTable":
            raise ConversionException(
                "XML is not from the expected table", "CalGainTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "CalGainTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "CalGainTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "CalGainTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "CalGainTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "CalGainTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "CalGainTable") from None

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "CalGainTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "CalGainTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalGainTable")
        return
        ...
        # .append("\n")
        # .append("<CalGainTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:clgn=\"http://Alma/XASDM/CalGainTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalGainTable http://almaobservatory.org/XML/XASDM/4/CalGainTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalGainTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<fit/>\n")
        # .append("<fitWeight/>\n")
        # .append("<totalGainValid/>\n")
        # .append("<totalFit/>\n")
        # .append("<totalFitWeight/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalGain")
        return
        ...
        #    for (CalGainRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalGain");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalGain");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalGain");
    # }
    #
    # //
    ...
    #     attributesSeq.add("calDataId"); attributesSeq.add("calReductionId"); attributesSeq.add("startValidTime"); attributesSeq.add("endValidTime"); attributesSeq.add("gain"); attributesSeq.add("gainValid"); attributesSeq.add("fit"); attributesSeq.add("fitWeight"); attributesSeq.add("totalGainValid"); attributesSeq.add("totalFit"); attributesSeq.add("totalFitWeight");
    #
    ...
    #    else {
    #        // Then it must be a "<CalGainTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalGainTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalGainTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalGain");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalGainTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalGain");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalGainTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalGainTable/Attributes#'. Invalid XML header '"+header+"'.", "CalGain");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalGain");
    #    }
    #
    ...
    #    this.checkAndAdd(CalGainRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "CalGain.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "CalGain.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the CalGain table", "CalGainTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalGainTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalGain.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalGain");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalGain");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "CalGain.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalGain found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CalGain.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CalGain.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (CalGain)"
                % directory,
                "CalGainTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for CalGain")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/CalGain.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalGain");
            #
            # File binFile = new File(directory+"/CalGain.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalGain");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalGain");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalGain");
        # }
        # }
            ...
            filePath = os.path.join(directory, "CalGain.xml")
