...
class CalGainTable:
    ...
    def toXML(self):
        ...
        result += '\n<CalGainTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clgn="http://Alma/XASDM/CalGainTable" xsi:schemaLocation="http://Alma/XASDM/CalGainTable http://almaobservatory.org/XML/XASDM/4/CalGainTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalGainTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<fit/>\n"
        result += "<fitWeight/>\n"
        result += "<totalGainValid/>\n"
        result += "<totalFit/>\n"
        result += "<totalFitWeight/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalGain"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalGain"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalGain")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("gain")

            attributesSeq.append("gainValid")

            attributesSeq.append("fit")

            attributesSeq.append("fitWeight")

            attributesSeq.append("totalGainValid")

            attributesSeq.append("totalFit")

            attributesSeq.append("totalFitWeight")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalGain table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalGainTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalGain"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "CalGain"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalGain")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalGainRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalGainTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalGain.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalGain.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalGain.bin")
