...
class ScaleTable:
    ...
    Attributes:
        ...
        scaleId (Tag): identifies a unique row in the table Scale. auto-incrementable, key.
    ...
    def toXML(self):
        ...
        result += '\n<ScaleTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:scale="http://Alma/XASDM/ScaleTable" xsi:schemaLocation="http://Alma/XASDM/ScaleTable http://almaobservatory.org/XML/XASDM/4/ScaleTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "ScaleTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='ScaleTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<scaleId/>\n"
        result += "<timeScale/>\n"
        result += "<crossDataScale/>\n"
        result += "<autoDataScale/>\n"
        result += "<weightType/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Scale"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Scale"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Scale")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("timeScale")

            attributesSeq.append("crossDataScale")

            attributesSeq.append("autoDataScale")

            attributesSeq.append("weightType")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Scale table
            # this is more insistant, just in case
            if hdrdom.nodeName != "ScaleTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Scale"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Scale"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Scale")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    ScaleRow.fromBin(eis, self, attributesSeq), self._checkRowUniqueness
                )
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Scale.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Scale.xml")
            ...
            mimeFilePath = os.path.join(directory, "Scale.bin")
