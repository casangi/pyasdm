...
class ScaleTable:
    ...
    scaleId Tag (auto-incrementable)     identifies a unique row in the table Scale. </TD>
    ...
    # A data structure to store the ScaleRow s.
    # In all cases we maintain a private list of ScaleRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  ScaleRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  scaleId to Tag(nextInt).
            x.setScaleId(Tag(nextInt, TagType.Scale))
    ...
    def __init__(self, container):
        """
        Create a ScaleTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Scale is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        homologues in x.  If a row is found that row else autoincrement x.\scaleId,
        add x to its table and returns x.

        returns a ScaleRow.
        x. A row to be added.
        """
        if not isinstance(x, ScaleRow):
            raise ValueError("x must be a  ScaleRow instance.")
    ...
    def newRow(self, timeScale, crossDataScale, autoDataScale, weightType):
        """
        Create a new ScaleRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getScaleId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of ScaleRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "ScaleTable":
            raise ConversionException(
                "XML is not from the expected table", "ScaleTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "ScaleTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <ScaleTable")
        return
        ...
        # .append("\n")
        # .append("<ScaleTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:scale=\"http://Alma/XASDM/ScaleTable\" xsi:schemaLocation=\"http://Alma/XASDM/ScaleTable http://almaobservatory.org/XML/XASDM/4/ScaleTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='ScaleTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<scaleId/>\n")
        # .append("<timeScale/>\n")
        # .append("<crossDataScale/>\n")
        # .append("<autoDataScale/>\n")
        # .append("<weightType/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Scale")
        return
        ...
        #    for (ScaleRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Scale");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Scale");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Scale");
    # }
    #
    # //
    ...
    #     attributesSeq.add("scaleId"); attributesSeq.add("timeScale"); attributesSeq.add("crossDataScale"); attributesSeq.add("autoDataScale"); attributesSeq.add("weightType");
    #
    ...
    #    else {
    #        // Then it must be a "<ScaleTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/ScaleTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/ScaleTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Scale");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/ScaleTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Scale");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/ScaleTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/ScaleTable/Attributes#'. Invalid XML header '"+header+"'.", "Scale");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Scale");
    #    }
    #
    ...
    #    this.checkAndAdd(ScaleRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for ScaleTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Scale.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Scale");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Scale");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Scale found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Scale.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Scale.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/Scale.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Scale");
            #
            # File binFile = new File(directory+"/Scale.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Scale");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Scale");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Scale");
        # }
