...
class SysPowerTable:
    ...
    # A data structure to store the SysPowerRow s.
    # In all cases we maintain a private list of SysPowerRow s.
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x The SysPowerRow to be inserted.
            ...
            while k0 != (k1 - 1):
                if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                    if rowlist[k0].equalByRequiredValue(x):
                        ...
                        raise DuplicateKey(
                            "DuplicateKey exception in ", "SysPowerTable"
                        )
    ...
    def __init__(self, container):
        """
        Create a SysPowerTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("SysPower is not present in memory, setting from file")
    ...
    def add(self, x):
        """
        Add a row.
        x the SysPowerRow to be added.

        return a SysPowerRow. If the table contains a SysPowerRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added SysPowerRow, otherwise x is returned.

        raises  DuplicateKey when the table contains a SysPowerRow with a key equal to the key in x but having
        ...
        """
        if not isinstance(x, SysPowerRow):
            raise ValueError("x must be a  SysPowerRow instance.")
    ...
    def newRow(self, antennaId, spectralWindowId, feedId, timeInterval, numReceptor):
        """
        Create a new SysPowerRow initialized to the specified values.
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of SysPowerRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId, feedId):
        ...
        return a list  of SysPowerRow. A None value is returned if the table contains
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "SysPowerTable":
            raise ConversionException(
                "XML is not from the expected table", "SysPowerTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "SysPowerTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <SysPowerTable")
        return
        ...
        # .append("\n")
        # .append("<SysPowerTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:syspwr=\"http://Alma/XASDM/SysPowerTable\" xsi:schemaLocation=\"http://Alma/XASDM/SysPowerTable http://almaobservatory.org/XML/XASDM/4/SysPowerTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='SysPowerTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<switchedPowerDifference/>\n")
        # .append("<switchedPowerSum/>\n")
        # .append("<requantizerGain/>\n")
        # .append("</Attributes>\n")
        # .append("</SysPowerTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for SysPower")
        return
        ...
        #    for (SysPowerRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "SysPower");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "SysPower");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "SysPower");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("feedId"); attributesSeq.add("timeInterval"); attributesSeq.add("numReceptor");
    #     attributesSeq.add("switchedPowerDifference");  attributesSeq.add("switchedPowerSum");  attributesSeq.add("requantizerGain");
    ...
    #    else {
    #        // Then it must be a "<SysPowerTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/SysPowerTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/SysPowerTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SysPower");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/SysPowerTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SysPower");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/SysPowerTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/SysPowerTable/Attributes#'. Invalid XML header '"+header+"'.", "SysPower");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "SysPower");
    #    }
    #
    ...
    #    this.checkAndAdd(SysPowerRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for SysPowerTable")
        return

        # java code looks like this
        # File file = new File(directory+"/SysPower.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "SysPower");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "SysPower");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "SysPower found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "SysPower.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "SysPower.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/SysPower.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "SysPower");
            #
            # File binFile = new File(directory+"/SysPower.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "SysPower");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SysPower");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SysPower");
        # }
