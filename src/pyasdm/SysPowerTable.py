...
class SysPowerTable:
    ...
    def toXML(self):
        ...
        result += '\n<SysPowerTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:syspwr="http://Alma/XASDM/SysPowerTable" xsi:schemaLocation="http://Alma/XASDM/SysPowerTable http://almaobservatory.org/XML/XASDM/4/SysPowerTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='SysPowerTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "SysPower"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "SysPower"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "SysPower")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("switchedPowerDifference")
            ...
            # c++ and Java just assume it then must be a SysPower table
            # this is more insistant, just in case
            if hdrdom.nodeName != "SysPowerTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "SysPower"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("SysPower")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    SysPowerRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "SysPowerTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "SysPower.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "SysPower.xml")
            ...
            mimeFilePath = os.path.join(directory, "SysPower.bin")
