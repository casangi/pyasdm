...
class AntennaTable:
    ...
    antennaId Tag (auto-incrementable)     identifies a unique row in the table. </TD>
    ...
    name  str  the antenna's name.

    antennaMake  AntennaMake  the antenna's make.

    antennaType  AntennaType  the antenna's type.

    dishDiameter  Length  the diameter of the main reflector.

    position  Length []   3  the antenna's position.

    offset  Length []   3  the position's offset.

    time  ArrayTime  the time of position's measurement.

    stationId  Tag  refers to the station where this antenna is located (i.e. one row in the Station table).



                 Value (Optional)

    assocAntennaId  Tag  refers to an associate antenna (i.e. one row in the Antenna table).
    ...
    # A data structure to store the AntennaRow s.
    # In all cases we maintain a private list of AntennaRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  AntennaRow s. Not needed in python.
    ...
    # The tolerance which will be used on dishDiameter during an add operation on the table
    _dishDiameterEqTolerance = Length(0.0)

    def setDishDiameterEqTolerance(self, tolerance):
        """
        A setter for the tolerance on dishDiameter
        """
        if not isinstance(tolerance, Length):
            print("tolerance must be a  Length instance")

        self._dishDiameterEqTolerance = Length(tolerance)

    def getDishDiameterEqTolerance(self):
        """
        A getter for the tolerance on dishDiameter
        Returns the tolerance as a  Length
        """
        return self._dishDiameterEqTolerance
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  antennaId to Tag(nextInt).
            x.setAntennaId(Tag(nextInt, TagType.Antenna))
    ...
    def __init__(self, container):
        """
        Create a AntennaTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("AntennaTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("AntennaTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Antenna is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "AntennaTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = AntennaRow(self)
        return thisRow

    def add(self, x):
        """
        Look up the table for a row whose noautoincrementable attributes are matching their
        homologues in x.  If a row is found that row else autoincrement x.\antennaId,
        add x to its table and returns x.

        returns a AntennaRow.
        x. A row to be added.
        """
        if not isinstance(x, AntennaRow):
            raise ValueError("x must be a  AntennaRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new AntennaRow initialized to the specified values.
        ...
        thisRow.setAntennaType(antennaType)

        thisRow.setDishDiameter(dishDiameter)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new AntennaRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new AntennaRow with default values for its attributes.
        """

        return AntennaRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if (
            ...
            raise UniquenessViolationException(
                "Uniqueness violation exception in table AntennaTable"
            )

        if self.getRowByKey(x.getAntennaId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "AntennaTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of AntennaRow
        """
        return self._privateRows

    def getRowByKey(self, antennaId):
        """
    ...
    def toXML(self):
        ...
        result += '<AntennaTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:antnn="http://Alma/XASDM/AntennaTable" xsi:schemaLocation="http://Alma/XASDM/AntennaTable http://almaobservatory.org/XML/XASDM/4/AntennaTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a Antenna (AntennaTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "AntennaTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "AntennaTable":
            raise ConversionException(
                "XML is not from the expected table", "AntennaTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "AntennaTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "AntennaTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "AntennaTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "AntennaTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "AntennaTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "AntennaTable") from None

                except UniquenessViolationException as exc:
                    msg = (
                        "UniquenessViolationException in row in AntennaTable : %s"
                        % str(exc)
                    )

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "AntennaTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "AntennaTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <AntennaTable")
        return
        ...
        # .append("\n")
        # .append("<AntennaTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:antnn=\"http://Alma/XASDM/AntennaTable\" xsi:schemaLocation=\"http://Alma/XASDM/AntennaTable http://almaobservatory.org/XML/XASDM/4/AntennaTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='AntennaTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Antenna")
        return
        ...
        #    for (AntennaRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Antenna");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Antenna");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Antenna");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("name"); attributesSeq.add("antennaMake"); attributesSeq.add("antennaType"); attributesSeq.add("dishDiameter"); attributesSeq.add("position"); attributesSeq.add("offset"); attributesSeq.add("time"); attributesSeq.add("stationId");
    #     attributesSeq.add("assocAntennaId");
    ...
    #    else {
    #        // Then it must be a "<AntennaTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/AntennaTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/AntennaTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Antenna");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/AntennaTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Antenna");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/AntennaTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/AntennaTable/Attributes#'. Invalid XML header '"+header+"'.", "Antenna");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Antenna");
    #    }
    #
    ...
    #    this.checkAndAdd(AntennaRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "Antenna.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "Antenna.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the Antenna table", "AntennaTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for AntennaTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Antenna.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Antenna");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Antenna");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "Antenna.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Antenna found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Antenna.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Antenna.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (Antenna)"
                % directory,
                "AntennaTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for Antenna")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/Antenna.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Antenna");
            #
            # File binFile = new File(directory+"/Antenna.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Antenna");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Antenna");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Antenna");
        # }
        # }
            ...
            filePath = os.path.join(directory, "Antenna.xml")
