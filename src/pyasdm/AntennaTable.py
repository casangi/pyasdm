...
class AntennaTable:
    ...
    Attributes:
        ...
        antennaId (Tag): identifies a unique row in the table. auto-incrementable, key.
        ...
        position (Length [] ): Array(3) the antenna's position.

        offset (Length [] ): Array(3) the position's offset.
        ...
        assocAntennaId (Tag): refers to an associate antenna (i.e. one row in the Antenna table). Optional.
    ...
    def toXML(self):
        ...
        result += '\n<AntennaTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:antnn="http://Alma/XASDM/AntennaTable" xsi:schemaLocation="http://Alma/XASDM/AntennaTable http://almaobservatory.org/XML/XASDM/4/AntennaTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "AntennaTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='AntennaTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Antenna"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Antenna"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Antenna")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("antennaType")

            attributesSeq.append("dishDiameter")
            ...
            attributesSeq.append("stationId")

            attributesSeq.append("assocAntennaId")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Antenna table
            # this is more insistant, just in case
            if hdrdom.nodeName != "AntennaTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Antenna"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Antenna"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Antenna")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    AntennaRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Antenna.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Antenna.xml")
            ...
            mimeFilePath = os.path.join(directory, "Antenna.bin")
