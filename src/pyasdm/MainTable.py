...
class MainTable:
    ...
    def toXML(self):
        ...
        result += '\n<MainTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:main="http://Alma/XASDM/MainTable" xsi:schemaLocation="http://Alma/XASDM/MainTable http://almaobservatory.org/XML/XASDM/4/MainTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='MainTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Main"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Main"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Main")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("timeSampling")
            ...
            attributesSeq.append("dataSize")

            attributesSeq.append("dataUID")
            ...
            # c++ and Java just assume it then must be a Main table
            # this is more insistant, just in case
            if hdrdom.nodeName != "MainTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Main"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Main"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Main")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    MainRow.fromBin(eis, self, attributesSeq), self._checkRowUniqueness
                )
            ...
            raise ConversionException(msg, "MainTable") from None
        except Exception as exc:
            byteStream.close()
            eis.close()
            raise ConversionException(
                "Error while reading binary data, the exception was " + str(exc), "Main"
            ) from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Main.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Main.xml")
            ...
            mimeFilePath = os.path.join(directory, "Main.bin")
