...
class ProcessorTable:
    ...
    Attributes:
        ...
        processorId (Tag): Processor identifier auto-incrementable, key.
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        if not skipUniquenessCheck:
            if (
                self.lookup(
                    x.getModeId(), x.getProcessorType(), x.getProcessorSubType()
                )
                is not None
            ):
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "ProcessorTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='ProcessorTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Processor"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Processor"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Processor")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("modeId")

            attributesSeq.append("processorType")

            attributesSeq.append("processorSubType")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Processor table
            # this is more insistant, just in case
            if hdrdom.nodeName != "ProcessorTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Processor"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Processor")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    ProcessorRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Processor.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Processor.xml")
            ...
            mimeFilePath = os.path.join(directory, "Processor.bin")
