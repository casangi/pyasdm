...
class ProcessorTable:
    ...
    processorId Tag (auto-incrementable)     Processor identifier </TD>
    ...
    # A data structure to store the ProcessorRow s.
    # In all cases we maintain a private list of ProcessorRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  ProcessorRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  processorId to Tag(nextInt).
            x.setProcessorId(Tag(nextInt, TagType.Processor))
    ...
    def __init__(self, container):
        """
        Create a ProcessorTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Processor is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        homologues in x.  If a row is found that row else autoincrement x.\processorId,
        add x to its table and returns x.

        returns a ProcessorRow.
        x. A row to be added.
        """
        if not isinstance(x, ProcessorRow):
            raise ValueError("x must be a  ProcessorRow instance.")

        aRow = self.lookup(x.getModeId(), x.getProcessorType(), x.getProcessorSubType())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, modeId, processorType, processorSubType):
        """
        Create a new ProcessorRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if (
            self.lookup(x.getModeId(), x.getProcessorType(), x.getProcessorSubType())
            is not None
        ):
        ...
        if self.getRowByKey(x.getProcessorId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of ProcessorRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "ProcessorTable":
            raise ConversionException(
                "XML is not from the expected table", "ProcessorTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "ProcessorTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <ProcessorTable")
        return
        ...
        # .append("\n")
        # .append("<ProcessorTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:prcssr=\"http://Alma/XASDM/ProcessorTable\" xsi:schemaLocation=\"http://Alma/XASDM/ProcessorTable http://almaobservatory.org/XML/XASDM/4/ProcessorTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='ProcessorTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<processorSubType/>\n")

        # .append("</Attributes>\n")
        # .append("</ProcessorTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Processor")
        return
        ...
        #    for (ProcessorRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Processor");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Processor");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Processor");
    # }
    #
    # //
    ...
    #     attributesSeq.add("processorId"); attributesSeq.add("modeId"); attributesSeq.add("processorType"); attributesSeq.add("processorSubType");
    #
    ...
    #    else {
    #        // Then it must be a "<ProcessorTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/ProcessorTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/ProcessorTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Processor");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/ProcessorTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Processor");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/ProcessorTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/ProcessorTable/Attributes#'. Invalid XML header '"+header+"'.", "Processor");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Processor");
    #    }
    #
    ...
    #    this.checkAndAdd(ProcessorRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for ProcessorTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Processor.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Processor");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Processor");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Processor found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Processor.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Processor.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/Processor.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Processor");
            #
            # File binFile = new File(directory+"/Processor.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Processor");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Processor");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Processor");
        # }
