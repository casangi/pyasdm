...
class EphemerisTable:
    ...
    def toXML(self):
        ...
        result += '\n<EphemerisTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ephmrs="http://Alma/XASDM/EphemerisTable" xsi:schemaLocation="http://Alma/XASDM/EphemerisTable http://almaobservatory.org/XML/XASDM/4/EphemerisTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='EphemerisTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Ephemeris"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Ephemeris"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Ephemeris")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("ephemerisId")

            attributesSeq.append("observerLocation")

            attributesSeq.append("equinoxEquator")

            attributesSeq.append("numPolyDir")

            attributesSeq.append("dir")

            attributesSeq.append("numPolyDist")

            attributesSeq.append("distance")

            attributesSeq.append("timeOrigin")

            attributesSeq.append("origin")

            attributesSeq.append("numPolyRadVel")

            attributesSeq.append("radVel")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Ephemeris table
            # this is more insistant, just in case
            if hdrdom.nodeName != "EphemerisTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Ephemeris"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Ephemeris")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    EphemerisRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "EphemerisTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Ephemeris.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Ephemeris.xml")
            ...
            mimeFilePath = os.path.join(directory, "Ephemeris.bin")
