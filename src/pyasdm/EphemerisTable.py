...
class EphemerisTable(Representable):
    """
        ...
        timeInterval ArrayTimeInterval interval of time during which the data are relevant.

        ephemerisId int identifies a collection of rows in the table.



                      Value (Mandatory)

        observerLocation double []   3  a triple of double precision values defining the observer location. This triple contains in that order the longitude, the latitude and the altitude of the observer :
    <ul>
    ...
    A triple with all its elements equal to 0.0 will mean that a geocentric coordinate system is in use instead of a topocentric one.
        ...
        equinoxEquator double  epoch at which equator and equinox were calculated for ephemeris. Expresses a year as a decimal value (J2000 would be represented as 2000.0).

        numPolyDir int  the number of coefficients of the polynomial stored in phaseDir. It has to be \f$ \ge 1 \f$.

        dir double []  []   numPolyDir, 2  the ephemeris direction expressed in radian. The nominal entry in the phaseDir, delayDir, or ReferenceDir in the Field table serves as additional offset to the direction described by "dir". The actual direction is obtained by composition, e.g. actual phase direction = [phasDir value from Field table] + [dir].
    ...
    \f[ dt = t - timeOrigin \f]
        ...
        numPolyDist int  the number of coefficients of the polynomial stored in distance. It has to be \f$ \ge 1 \f$.

        distance double []   numPolyDist  the coefficiens of the polynomial used to calculate the distance, expressed in meter,  to the object from the position of the antenna along the given direction. This distance is the result of the sum :
    ...
    \f[ dt = t - timeOrigin \f].
        ...
        numPolyRadVel int  the number of coefficients of the polynomial stored in radVel . It has to be \f$ \ge 1 \f$.

        radVel double []   numPolyRadVel   the coefficients of a polynomial expressing a radial velocity as a function of the time expressed in m/s. The time origin used to tabulate the polynomial is stored in timeOrigin.
    ...
    # A list to store the EphemerisRow instances
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x is a EphemerisRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a EphemerisTable attached to container, which must be a ASDM instance
    ...
    def add(self, row):
        """
        Add a row.
        row the EphemerisRow to be added.

        return a EphemerisRow. If the table contains a EphemerisRow whose attributes (key and mandatory values) are equal to those in row
        then this returns that previously added EphemerisRow, otherwise row is returned.

        raises DuplicateKey when the table contains a EphemerisRow with a key equal to the key in row but having
    ...
    def newRow(
        ...
        """
        Create a new EphemerisRow. The new row is not added to this table, but it does know about it.
    ...
    def _checkAndAdd(self, newrow):
        ...
        thisKey = self.Key(newrow.getEphemerisId())
    ...
    def get(self):
        """
        Get all rows as a list of EphemerisRow
        """
    ...
    def getByContext(self, ephemerisId):
        ...
        The context is defined by the value of ( ephemerisId ).
        A None value is returned if the table contains no rows for the given
        ( ephemerisId ).
        """
        thisKey = EphemerisTable.Key(ephemerisId)
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "EphemerisTable":
            raise ConversionException(
                "XML is not from a the expected table", "EphemerisTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException(
                "schemaVersion not found in XML", "EphemerisTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "EphemerisTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("Ephemeris.xml is empty", "EphemerisTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'Ephemeris.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'Ephemeris.xml'.
