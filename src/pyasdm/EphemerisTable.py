...
class EphemerisTable:
    ...
    _tableName = "Ephemeris"

    # The list of field names that make up key 'key'.
    _key = ["timeInterval", "ephemerisId"]
    ...
    # A data structure to store the EphemerisRow s.
    # In all cases we maintain a private list of EphemerisRow s.
    _privateRows = []
    ...
    def Key(self, ephemerisId):
        ...
        result += str(ephemerisId) + "_"

        return result

    def insertByStartTime(self, x, rowlist):
        """
        Insert a EphemerisRow in a list of EphemerisRow so that it's ordered by ascending start time.

        x The EphemerisRow to be inserted.
        ...
        if (len(rowlist) == 0) or (
            ...
            while k0 != (k1 - 1):
                if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                    if rowlist[k0].equalByRequiredValue(x):
                        ...
                        raise DuplicateKey(
                            "DuplicateKey exception in ", "EphemerisTable"
                        )
            ...
            if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                if rowlist[k0].equalByRequiredValue(x):
                    ...
                    raise DuplicateKey("DuplicateKey exception in ", "EphemerisTable")
            elif xTimeStart == rowlist[k1].getTimeInterval().getStart().get():
                if rowlist[k1].equalByRequiredValue(x):
                    # this row already exists at k1, nothing to insert or add, return that row
                    return rowlist[k1]
    ...
    def __init__(self, container):
        """
        Create a EphemerisTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("EphemerisTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("EphemerisTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Ephemeris is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "EphemerisTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = EphemerisRow(self)
        return thisRow

    def add(self, x):
        """
        Add a row.
        x the EphemerisRow to be added.

        return a EphemerisRow. If the table contains a EphemerisRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added EphemerisRow, otherwise x is returned.
        ...
        """
        if not isinstance(x, EphemerisRow):
            raise ValueError("x must be a  EphemerisRow instance.")

        # get the key for x
        keystr = self.Key(x.getEphemerisId())
    ...
    def newRow(
        ...
        """
        Create a new EphemerisRow initialized to the specified values.
        ...
        thisRow.setEphemerisId(ephemerisId)

        thisRow.setObserverLocation(observerLocation)

        thisRow.setEquinoxEquator(equinoxEquator)

        thisRow.setNumPolyDir(numPolyDir)

        thisRow.setDir(dir)

        thisRow.setNumPolyDist(numPolyDist)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new EphemerisRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new EphemerisRow with default values for its attributes.
        """

        return EphemerisRow(self, row)
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of EphemerisRow
        """
        return self._privateRows

    def getByContext(self, ephemerisId):
        """
        Returns all the rows sorted by ascending startTime for a given context.
        The context is defined by a value of ( ephemerisId ).

        return a list  of EphemerisRow. A None value is returned if the table contains
        no EphemerisRow for the given ( ephemerisId ).
        """

        keystr = self.Key(ephemerisId)
    ...
    def toXML(self):
        ...
        result += '<EphemerisTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ephmrs="http://Alma/XASDM/EphemerisTable" xsi:schemaLocation="http://Alma/XASDM/EphemerisTable http://almaobservatory.org/XML/XASDM/4/EphemerisTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
        ...
        for thisRow in self._privateRows:
            ...
            result += " "
        result += "</EphemerisTable>"
        return result

    def fromXML(self, xmlstr):
        """
        Populate this table from the content of a XML document that is required to
        conform to the XML schema defined for a Ephemeris (EphemerisTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "EphemerisTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "EphemerisTable":
            raise ConversionException(
                "XML is not from the expected table", "EphemerisTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "EphemerisTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "EphemerisTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "EphemerisTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "EphemerisTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "EphemerisTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "EphemerisTable") from None

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "EphemerisTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "EphemerisTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <EphemerisTable")
        return
        ...
        # .append("\n")
        # .append("<EphemerisTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:ephmrs=\"http://Alma/XASDM/EphemerisTable\" xsi:schemaLocation=\"http://Alma/XASDM/EphemerisTable http://almaobservatory.org/XML/XASDM/4/EphemerisTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='EphemerisTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<timeInterval/>\n")
        # .append("<ephemerisId/>\n")
        # .append("<observerLocation/>\n")
        # .append("<equinoxEquator/>\n")
        # .append("<numPolyDir/>\n")
        # .append("<dir/>\n")
        # .append("<numPolyDist/>\n")
        # .append("<distance/>\n")
        ...
        # .append("<numPolyRadVel/>\n")
        # .append("<radVel/>\n")
        # .append("</Attributes>\n")
        # .append("</EphemerisTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Ephemeris")
        return
        ...
        #    for (EphemerisRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Ephemeris");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Ephemeris");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Ephemeris");
    # }
    #
    # //
    ...
    #     attributesSeq.add("timeInterval"); attributesSeq.add("ephemerisId"); attributesSeq.add("observerLocation"); attributesSeq.add("equinoxEquator"); attributesSeq.add("numPolyDir"); attributesSeq.add("dir"); attributesSeq.add("numPolyDist"); attributesSeq.add("distance"); attributesSeq.add("timeOrigin"); attributesSeq.add("origin");
    #     attributesSeq.add("numPolyRadVel");  attributesSeq.add("radVel");
    ...
    #    else {
    #        // Then it must be a "<EphemerisTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/EphemerisTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/EphemerisTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Ephemeris");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/EphemerisTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Ephemeris");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/EphemerisTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/EphemerisTable/Attributes#'. Invalid XML header '"+header+"'.", "Ephemeris");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Ephemeris");
    #    }
    #
    ...
    #    this.checkAndAdd(EphemerisRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "Ephemeris.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "Ephemeris.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the Ephemeris table", "EphemerisTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for EphemerisTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Ephemeris.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Ephemeris");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Ephemeris");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "Ephemeris.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Ephemeris found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Ephemeris.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Ephemeris.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (Ephemeris)"
                % directory,
                "EphemerisTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for Ephemeris")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/Ephemeris.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Ephemeris");
            #
            # File binFile = new File(directory+"/Ephemeris.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Ephemeris");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Ephemeris");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Ephemeris");
        # }
        # }
            ...
            filePath = os.path.join(directory, "Ephemeris.xml")
