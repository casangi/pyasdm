...
class CalPhaseTable:
    ...
    def toXML(self):
        ...
        result += '\n<CalPhaseTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clphas="http://Alma/XASDM/CalPhaseTable" xsi:schemaLocation="http://Alma/XASDM/CalPhaseTable http://almaobservatory.org/XML/XASDM/4/CalPhaseTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalPhaseTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalPhase"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalPhase"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalPhase")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            # c++ and Java just assume it then must be a CalPhase table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalPhaseTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalPhase"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalPhase")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalPhaseRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalPhaseTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalPhase.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalPhase.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalPhase.bin")
