...
class CalPointingTable:
    ...
    def toXML(self):
        ...
        result += '\n<CalPointingTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clpntg="http://Alma/XASDM/CalPointingTable" xsi:schemaLocation="http://Alma/XASDM/CalPointingTable http://almaobservatory.org/XML/XASDM/4/CalPointingTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalPointingTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<pointingMethod/>\n"
        result += "<numReceptor/>\n"
        result += "<polarizationTypes/>\n"
        result += "<collOffsetRelative/>\n"
        result += "<collOffsetAbsolute/>\n"
        result += "<collError/>\n"
        result += "<collOffsetTied/>\n"
        result += "<reducedChiSquared/>\n"

        result += "<averagedPolarizations/>\n"
        result += "<beamPA/>\n"
        result += "<beamPAError/>\n"
        result += "<beamPAWasFixed/>\n"
        result += "<beamWidth/>\n"
        result += "<beamWidthError/>\n"
        result += "<beamWidthWasFixed/>\n"
        result += "<offIntensity/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalPointing"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalPointing"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalPointing")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("pointingMethod")

            attributesSeq.append("numReceptor")

            attributesSeq.append("polarizationTypes")

            attributesSeq.append("collOffsetRelative")

            attributesSeq.append("collOffsetAbsolute")

            attributesSeq.append("collError")

            attributesSeq.append("collOffsetTied")

            attributesSeq.append("reducedChiSquared")

            attributesSeq.append("averagedPolarizations")

            attributesSeq.append("beamPA")

            attributesSeq.append("beamPAError")

            attributesSeq.append("beamPAWasFixed")

            attributesSeq.append("beamWidth")

            attributesSeq.append("beamWidthError")

            attributesSeq.append("beamWidthWasFixed")
            ...
            # c++ and Java just assume it then must be a CalPointing table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalPointingTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalPointing"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalPointing")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalPointingRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalPointingTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalPointing.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalPointing.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalPointing.bin")
