...
class FreqOffsetTable(Representable):
    """
    ...
    # A list to store the FreqOffsetRow instances
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x is a FreqOffsetRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a FreqOffsetTable attached to container, which must be a ASDM instance
    ...
    def add(self, row):
        """
        Add a row.
        row the FreqOffsetRow to be added.

        return a FreqOffsetRow. If the table contains a FreqOffsetRow whose attributes (key and mandatory values) are equal to those in row
        then this returns that previously added FreqOffsetRow, otherwise row is returned.

        raises DuplicateKey when the table contains a FreqOffsetRow with a key equal to the key in row but having
    ...
    def newRow(self, antennaId, spectralWindowId, timeInterval, feedId, offset):
        """
        Create a new FreqOffsetRow. The new row is not added to this table, but it does know about it.
    ...
    def get(self):
        """
        Get all rows as a list of FreqOffsetRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId, feedId):
        ...
        """
        thisKey = FreqOffsetTable.Key(antennaId, spectralWindowId, feedId)
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from a the expected table", "FreqOffsetTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException(
                "schemaVersion not found in XML", "FreqOffsetTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "FreqOffsetTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("FreqOffset.xml is empty", "FreqOffsetTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'FreqOffset.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'FreqOffset.xml'.
