...
class FreqOffsetTable:
    ...
    # A data structure to store the FreqOffsetRow s.
    # In all cases we maintain a private list of FreqOffsetRow s.
    _privateRows = []
    ...
    _offsetEqTolerance = Frequency(0.0)
    ...
    def setOffsetEqTolerance(self, tolerance):
        ...
        self._offsetEqTolerance = Frequency(tolerance)
    ...
    def insertByStartTime(self, x, rowlist):
        """
        Insert a FreqOffsetRow in a list of FreqOffsetRow so that it's ordered by ascending start time.

        x The FreqOffsetRow to be inserted.
        ...
        if (len(rowlist) == 0) or (
            ...
            while k0 != (k1 - 1):
                if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                    if rowlist[k0].equalByRequiredValue(x):
                        ...
                        raise DuplicateKey(
                            "DuplicateKey exception in ", "FreqOffsetTable"
                        )
            ...
            if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                if rowlist[k0].equalByRequiredValue(x):
                    ...
                    raise DuplicateKey("DuplicateKey exception in ", "FreqOffsetTable")
            elif xTimeStart == rowlist[k1].getTimeInterval().getStart().get():
                if rowlist[k1].equalByRequiredValue(x):
                    # this row already exists at k1, nothing to insert or add, return that row
                    return rowlist[k1]
    ...
    def __init__(self, container):
        """
        Create a FreqOffsetTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("FreqOffsetTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("FreqOffsetTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("FreqOffset is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "FreqOffsetTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = FreqOffsetRow(self)
        return thisRow

    def add(self, x):
        """
        Add a row.
        x the FreqOffsetRow to be added.

        return a FreqOffsetRow. If the table contains a FreqOffsetRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added FreqOffsetRow, otherwise x is returned.

        raises  DuplicateKey when the table contains a FreqOffsetRow with a key equal to the key in x but having
        ...
        """
        if not isinstance(x, FreqOffsetRow):
            raise ValueError("x must be a  FreqOffsetRow instance.")
    ...
    def newRow(self, antennaId, spectralWindowId, timeInterval, feedId, offset):
        """
        Create a new FreqOffsetRow initialized to the specified values.
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new FreqOffsetRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new FreqOffsetRow with default values for its attributes.
        """

        return FreqOffsetRow(self, row)
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of FreqOffsetRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId, feedId):
        ...
        return a list  of FreqOffsetRow. A None value is returned if the table contains
    ...
    def toXML(self):
        ...
        result += '<FreqOffsetTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:frqoff="http://Alma/XASDM/FreqOffsetTable" xsi:schemaLocation="http://Alma/XASDM/FreqOffsetTable http://almaobservatory.org/XML/XASDM/4/FreqOffsetTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
        ...
        for thisRow in self._privateRows:
            ...
            result += " "
        result += "</FreqOffsetTable>"
        return result

    def fromXML(self, xmlstr):
        """
        Populate this table from the content of a XML document that is required to
        conform to the XML schema defined for a FreqOffset (FreqOffsetTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "FreqOffsetTable".
        if (
            not xmldom.hasChildNodes()
            or xmldom.firstChild.nodeName != "FreqOffsetTable"
        ):
            raise ConversionException(
                "XML is not from the expected table", "FreqOffsetTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "FreqOffsetTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "FreqOffsetTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "FreqOffsetTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "FreqOffsetTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "FreqOffsetTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "FreqOffsetTable") from None

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "FreqOffsetTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "FreqOffsetTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <FreqOffsetTable")
        return
        ...
        # .append("\n")
        # .append("<FreqOffsetTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:frqoff=\"http://Alma/XASDM/FreqOffsetTable\" xsi:schemaLocation=\"http://Alma/XASDM/FreqOffsetTable http://almaobservatory.org/XML/XASDM/4/FreqOffsetTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='FreqOffsetTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for FreqOffset")
        return
        ...
        #    for (FreqOffsetRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "FreqOffset");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "FreqOffset");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "FreqOffset");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("timeInterval"); attributesSeq.add("feedId"); attributesSeq.add("offset");
    #
    ...
    #    else {
    #        // Then it must be a "<FreqOffsetTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/FreqOffsetTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/FreqOffsetTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "FreqOffset");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/FreqOffsetTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "FreqOffset");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/FreqOffsetTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/FreqOffsetTable/Attributes#'. Invalid XML header '"+header+"'.", "FreqOffset");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "FreqOffset");
    #    }
    #
    ...
    #    this.checkAndAdd(FreqOffsetRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "FreqOffset.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "FreqOffset.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the FreqOffset table", "FreqOffsetTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for FreqOffsetTable")
        return

        # java code looks like this
        # File file = new File(directory+"/FreqOffset.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "FreqOffset");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "FreqOffset");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "FreqOffset.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "FreqOffset found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "FreqOffset.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "FreqOffset.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (FreqOffset)"
                % directory,
                "FreqOffsetTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for FreqOffset")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/FreqOffset.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "FreqOffset");
            #
            # File binFile = new File(directory+"/FreqOffset.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "FreqOffset");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "FreqOffset");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "FreqOffset");
        # }
        # }
            ...
            filePath = os.path.join(directory, "FreqOffset.xml")
