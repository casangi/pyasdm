...
class FreqOffsetTable:
    ...
    def toXML(self):
        ...
        result += '\n<FreqOffsetTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:frqoff="http://Alma/XASDM/FreqOffsetTable" xsi:schemaLocation="http://Alma/XASDM/FreqOffsetTable http://almaobservatory.org/XML/XASDM/4/FreqOffsetTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='FreqOffsetTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "FreqOffset"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "FreqOffset"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "FreqOffset")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            # c++ and Java just assume it then must be a FreqOffset table
            # this is more insistant, just in case
            if hdrdom.nodeName != "FreqOffsetTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "FreqOffset"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("FreqOffset")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    FreqOffsetRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "FreqOffsetTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "FreqOffset.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "FreqOffset.xml")
            ...
            mimeFilePath = os.path.join(directory, "FreqOffset.bin")
