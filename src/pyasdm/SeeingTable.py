...
class SeeingTable:
    ...
    timeInterval ArrayTimeInterval  </TD>
    ...
    numBaseLength (numBaseLength) int
    ...
    # A data structure to store the SeeingRow s.
    # In all cases we maintain a private list of SeeingRow s.
    _privateRows = []

    # the only key is a temporal key, this is a list of rows in temporal order
    _row = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x The SeeingRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a SeeingTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Seeing is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        """
        if not isinstance(x, SeeingRow):
            raise ValueError("x must be a  SeeingRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new SeeingRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        and no other fields in the key.
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of SeeingRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "SeeingTable":
            raise ConversionException(
                "XML is not from the expected table", "SeeingTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "SeeingTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <SeeingTable")
        return
        ...
        # .append("\n")
        # .append("<SeeingTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:sng=\"http://Alma/XASDM/SeeingTable\" xsi:schemaLocation=\"http://Alma/XASDM/SeeingTable http://almaobservatory.org/XML/XASDM/4/SeeingTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='SeeingTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Seeing")
        return
        ...
        #    for (SeeingRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Seeing");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Seeing");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Seeing");
    # }
    #
    # //
    ...
    #     attributesSeq.add("timeInterval"); attributesSeq.add("numBaseLength"); attributesSeq.add("baseLength"); attributesSeq.add("phaseRms"); attributesSeq.add("seeing"); attributesSeq.add("exponent");
    #
    ...
    #    else {
    #        // Then it must be a "<SeeingTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/SeeingTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/SeeingTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Seeing");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/SeeingTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Seeing");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/SeeingTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/SeeingTable/Attributes#'. Invalid XML header '"+header+"'.", "Seeing");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Seeing");
    #    }
    #
    ...
    #    this.checkAndAdd(SeeingRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for SeeingTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Seeing.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Seeing");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Seeing");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Seeing found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Seeing.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Seeing.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/Seeing.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Seeing");
            #
            # File binFile = new File(directory+"/Seeing.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Seeing");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Seeing");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Seeing");
        # }
