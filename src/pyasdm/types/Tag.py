...
class Tag:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 0:
            ...
            if isinstance(thisArg,int):
                ...
                raise ValueError("single Tag constructor argument is not an int or a Tag")
        elif len(args) == 2:
            thisTag = args[0]
            thisType = args[1]
            if isinstance(thisTag, int) and isinstance(thisType, TagType):
                ...
                raise ValueError("Tag constructor with two arguments must be an int with a TagType")
        else:
    ...
    def parseTag(s):
        ...
        if type is None:
            raise("Error: '" + s + "' has an invalid Tag type part '" + typeValue[0] + "'")

        value = 0
        try:
            value = int(typeValue[1])
        except ValueError as exc:
            raise ValueError("Error: '" + s + "' has an invalid Tag value part '" + typeValue[1] + "'") from None
        ...
        if isinstance(other, Tag):
            if (self.isNull() or other.isNull()):
                ...
                if (not self.isNull()) and (not other.isNull()):
                    result = (self._tag == other._tag) and (self._type.toString() == other._type.toString())
        return (result)
