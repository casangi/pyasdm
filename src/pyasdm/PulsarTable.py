...
class PulsarTable:
    ...
    def toXML(self):
        ...
        result += '\n<PulsarTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:plsr="http://Alma/XASDM/PulsarTable" xsi:schemaLocation="http://Alma/XASDM/PulsarTable http://almaobservatory.org/XML/XASDM/4/PulsarTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "PulsarTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='PulsarTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<phasePoly/>\n"
        result += "<timeSpan/>\n"
        result += "<startPhaseBin/>\n"
        result += "<endPhaseBin/>\n"
        result += "<dispersionMeasure/>\n"
        result += "<refFrequency/>\n"
        result += "</Attributes>\n"
        result += "</PulsarTable>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Pulsar"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Pulsar")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("pulsarId")

            attributesSeq.append("refTime")

            attributesSeq.append("refPulseFreq")

            attributesSeq.append("refPhase")

            attributesSeq.append("numBin")

            attributesSeq.append("numPoly")

            attributesSeq.append("phasePoly")

            attributesSeq.append("timeSpan")

            attributesSeq.append("startPhaseBin")

            attributesSeq.append("endPhaseBin")

            attributesSeq.append("dispersionMeasure")

            attributesSeq.append("refFrequency")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Pulsar table
            # this is more insistant, just in case
            if hdrdom.nodeName != "PulsarTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Pulsar"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Pulsar"
                )
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    PulsarRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Pulsar.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Pulsar.xml")
            ...
            mimeFilePath = os.path.join(directory, "Pulsar.bin")
