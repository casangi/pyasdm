...
class PulsarTable:
    ...
    # A data structure to store the PulsarRow s.
    # In all cases we maintain a private list of PulsarRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  PulsarRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  pulsarId to Tag(nextInt).
            x.setPulsarId(Tag(nextInt, TagType.Pulsar))
    ...
    def __init__(self, container):
        """
        Create a PulsarTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Pulsar is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, PulsarRow):
            raise ValueError("x must be a  PulsarRow instance.")

        aRow = self.lookup(
            x.getRefTime(), x.getRefPulseFreq(), x.getRefPhase(), x.getNumBin()
        )
        if aRow is not None:
            return aRow
    ...
    def newRow(self, refTime, refPulseFreq, refPhase, numBin):
        """
        Create a new PulsarRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getPulsarId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of PulsarRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "PulsarTable":
            raise ConversionException(
                "XML is not from the expected table", "PulsarTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "PulsarTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <PulsarTable")
        return
        ...
        # .append("\n")
        # .append("<PulsarTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:plsr=\"http://Alma/XASDM/PulsarTable\" xsi:schemaLocation=\"http://Alma/XASDM/PulsarTable http://almaobservatory.org/XML/XASDM/4/PulsarTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='PulsarTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<phasePoly/>\n")
        # .append("<timeSpan/>\n")
        # .append("<startPhaseBin/>\n")
        # .append("<endPhaseBin/>\n")
        # .append("<dispersionMeasure/>\n")
        # .append("<refFrequency/>\n")
        # .append("</Attributes>\n")
        # .append("</PulsarTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Pulsar")
        return
        ...
        #    for (PulsarRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Pulsar");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Pulsar");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Pulsar");
    # }
    #
    # //
    ...
    #     attributesSeq.add("pulsarId"); attributesSeq.add("refTime"); attributesSeq.add("refPulseFreq"); attributesSeq.add("refPhase"); attributesSeq.add("numBin");
    #     attributesSeq.add("numPoly");  attributesSeq.add("phasePoly");  attributesSeq.add("timeSpan");  attributesSeq.add("startPhaseBin");  attributesSeq.add("endPhaseBin");  attributesSeq.add("dispersionMeasure");  attributesSeq.add("refFrequency");
    ...
    #    else {
    #        // Then it must be a "<PulsarTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/PulsarTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/PulsarTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Pulsar");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/PulsarTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Pulsar");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/PulsarTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/PulsarTable/Attributes#'. Invalid XML header '"+header+"'.", "Pulsar");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Pulsar");
    #    }
    #
    ...
    #    this.checkAndAdd(PulsarRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for PulsarTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Pulsar.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Pulsar");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Pulsar");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Pulsar found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Pulsar.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Pulsar.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/Pulsar.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Pulsar");
            #
            # File binFile = new File(directory+"/Pulsar.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Pulsar");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Pulsar");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Pulsar");
        # }
