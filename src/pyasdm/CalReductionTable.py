...
class CalReductionTable:
    ...
    Attributes:
        ...
        calReductionId (Tag): identifies a unique row in the table. auto-incrementable, key.





        numApplied (int): the number of applied calibrations prior the reduction.

        appliedCalibrations (str [] ): Array(numApplied) the list of applied calibrations (one string per calibration).

        numParam (int): the number of listed parameters used for calibration.

        paramSet (str [] ): Array(numParam) the input parameters expressed as (keyword,value) pairs (one string per parameter).

        numInvalidConditions (int): the number of invalidating conditions.

        invalidConditions (InvalidatingCondition [] ): Array(numInvalidConditions) invalidating use cases (one string per case).
    ...
    def toXML(self):
        ...
        result += '\n<CalReductionTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:clred="http://Alma/XASDM/CalReductionTable" xsi:schemaLocation="http://Alma/XASDM/CalReductionTable http://almaobservatory.org/XML/XASDM/4/CalReductionTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    msg = "UniquenessViolationException while adding row : %s" % str(
                        exc
                    )
                    # raise a ConversionException using this msg
                    raise ConversionException(msg, "CalReductionTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalReductionTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<numApplied/>\n"
        result += "<appliedCalibrations/>\n"
        result += "<numParam/>\n"
        result += "<paramSet/>\n"
        result += "<numInvalidConditions/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalReduction"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalReduction"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalReduction")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numApplied")

            attributesSeq.append("appliedCalibrations")

            attributesSeq.append("numParam")

            attributesSeq.append("paramSet")

            attributesSeq.append("numInvalidConditions")

            attributesSeq.append("invalidConditions")

            attributesSeq.append("timeReduced")

            attributesSeq.append("messages")

            attributesSeq.append("software")

            attributesSeq.append("softwareVersion")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalReduction table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalReductionTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalReduction"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalReduction")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalReductionRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalReduction.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalReduction.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalReduction.bin")
