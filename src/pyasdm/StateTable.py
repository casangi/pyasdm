...
class StateTable:
    ...
    Attributes:
        ...
        stateId (Tag): identifies a unique row in the table. auto-incrementable, key.
        ...
        weight (float): used for combining data. Optional.
    ...
    def toXML(self):
        ...
        result += '\n<StateTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:state="http://Alma/XASDM/StateTable" xsi:schemaLocation="http://Alma/XASDM/StateTable http://almaobservatory.org/XML/XASDM/4/StateTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "StateTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='StateTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "State"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "State"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "State")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("calDeviceName")

            attributesSeq.append("sig")

            attributesSeq.append("ref")

            attributesSeq.append("onSky")

            attributesSeq.append("weight")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a State table
            # this is more insistant, just in case
            if hdrdom.nodeName != "StateTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "State"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "State"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("State")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    StateRow.fromBin(eis, self, attributesSeq), self._checkRowUniqueness
                )
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "State.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "State.xml")
            ...
            mimeFilePath = os.path.join(directory, "State.bin")
