...
class StateTable:
    ...
    stateId Tag (auto-incrementable)     identifies a unique row in the table. </TD>
    ...
    # A data structure to store the StateRow s.
    # In all cases we maintain a private list of StateRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  StateRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  stateId to Tag(nextInt).
            x.setStateId(Tag(nextInt, TagType.State))
    ...
    def __init__(self, container):
        """
        Create a StateTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("State is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        homologues in x.  If a row is found that row else autoincrement x.\stateId,
        add x to its table and returns x.

        returns a StateRow.
        x. A row to be added.
        """
        if not isinstance(x, StateRow):
            raise ValueError("x must be a  StateRow instance.")

        aRow = self.lookup(x.getCalDeviceName(), x.getSig(), x.getRef(), x.getOnSky())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, calDeviceName, sig, ref, onSky):
        """
        Create a new StateRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if (
            self.lookup(x.getCalDeviceName(), x.getSig(), x.getRef(), x.getOnSky())
            is not None
        ):
        ...
        if self.getRowByKey(x.getStateId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of StateRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "StateTable":
            raise ConversionException(
                "XML is not from the expected table", "StateTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "StateTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <StateTable")
        return
        ...
        # .append("\n")
        # .append("<StateTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:state=\"http://Alma/XASDM/StateTable\" xsi:schemaLocation=\"http://Alma/XASDM/StateTable http://almaobservatory.org/XML/XASDM/4/StateTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='StateTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for State")
        return
        ...
        #    for (StateRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "State");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "State");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "State");
    # }
    #
    # //
    ...
    #     attributesSeq.add("stateId"); attributesSeq.add("calDeviceName"); attributesSeq.add("sig"); attributesSeq.add("ref"); attributesSeq.add("onSky");
    #     attributesSeq.add("weight");
    ...
    #    else {
    #        // Then it must be a "<StateTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/StateTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/StateTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "State");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/StateTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "State");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/StateTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/StateTable/Attributes#'. Invalid XML header '"+header+"'.", "State");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "State");
    #    }
    #
    ...
    #    this.checkAndAdd(StateRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for StateTable")
        return

        # java code looks like this
        # File file = new File(directory+"/State.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "State");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "State");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "State found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "State.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "State.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/State.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "State");
            #
            # File binFile = new File(directory+"/State.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "State");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "State");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "State");
        # }
