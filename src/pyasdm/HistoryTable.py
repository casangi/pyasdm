...
class HistoryTable:
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <HistoryTable")
        return
        ...
        # .append("\n")
        # .append("<HistoryTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:histry=\"http://Alma/XASDM/HistoryTable\" xsi:schemaLocation=\"http://Alma/XASDM/HistoryTable http://almaobservatory.org/XML/XASDM/4/HistoryTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='HistoryTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for History")
        return
        ...
        #    for (HistoryRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "History");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "History");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "History");
    # }
    #
    # //
    ...
    #     attributesSeq.add("execBlockId"); attributesSeq.add("time"); attributesSeq.add("message"); attributesSeq.add("priority"); attributesSeq.add("origin"); attributesSeq.add("objectId"); attributesSeq.add("application"); attributesSeq.add("cliCommand"); attributesSeq.add("appParms");
    #
    ...
    #    else {
    #        // Then it must be a "<HistoryTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/HistoryTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/HistoryTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "History");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/HistoryTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "History");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/HistoryTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/HistoryTable/Attributes#'. Invalid XML header '"+header+"'.", "History");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "History");
    #    }
    #
    ...
    #    this.checkAndAdd(HistoryRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for HistoryTable")
        return

        # java code looks like this
        # File file = new File(directory+"/History.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "History");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "History");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "History found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/History.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "History");
            #
            # File binFile = new File(directory+"/History.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "History");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "History");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "History");
        # }
