...
class HolographyTable:
    ...
    holographyId Tag (auto-incrementable)     identifies a unique row in the table. </TD>
    ...
    numCorr (numCorr) int  the number of stored correlations.

    type  HolographyChannelType []   numCorr  identifies the types of the correlation signals.
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <HolographyTable")
        return
        ...
        # .append("\n")
        # .append("<HolographyTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:hologr=\"http://Alma/XASDM/HolographyTable\" xsi:schemaLocation=\"http://Alma/XASDM/HolographyTable http://almaobservatory.org/XML/XASDM/4/HolographyTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='HolographyTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Holography")
        return
        ...
        #    for (HolographyRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Holography");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Holography");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Holography");
    # }
    #
    # //
    ...
    #     attributesSeq.add("holographyId"); attributesSeq.add("distance"); attributesSeq.add("focus"); attributesSeq.add("numCorr"); attributesSeq.add("type");
    #
    ...
    #    else {
    #        // Then it must be a "<HolographyTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/HolographyTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/HolographyTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Holography");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/HolographyTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Holography");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/HolographyTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/HolographyTable/Attributes#'. Invalid XML header '"+header+"'.", "Holography");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Holography");
    #    }
    #
    ...
    #    this.checkAndAdd(HolographyRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for HolographyTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Holography.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Holography");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Holography");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Holography found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for Holography")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/Holography.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Holography");
            #
            # File binFile = new File(directory+"/Holography.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Holography");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Holography");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Holography");
        # }
