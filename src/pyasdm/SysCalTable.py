...
class SysCalTable:
    ...
    def toXML(self):
        ...
        result += '\n<SysCalTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:syscal="http://Alma/XASDM/SysCalTable" xsi:schemaLocation="http://Alma/XASDM/SysCalTable http://almaobservatory.org/XML/XASDM/4/SysCalTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='SysCalTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<tantSpectrum/>\n"
        result += "<tantTsysFlag/>\n"
        result += "<tantTsysSpectrum/>\n"
        result += "<phaseDiffFlag/>\n"
        result += "<phaseDiffSpectrum/>\n"
        result += "</Attributes>\n"
        result += "</SysCalTable>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "SysCal"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "SysCal"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "SysCal")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("tcalFlag")

            attributesSeq.append("tcalSpectrum")

            attributesSeq.append("trxFlag")

            attributesSeq.append("trxSpectrum")

            attributesSeq.append("tskyFlag")

            attributesSeq.append("tskySpectrum")

            attributesSeq.append("tsysFlag")

            attributesSeq.append("tsysSpectrum")

            attributesSeq.append("tantFlag")

            attributesSeq.append("tantSpectrum")

            attributesSeq.append("tantTsysFlag")

            attributesSeq.append("tantTsysSpectrum")

            attributesSeq.append("phaseDiffFlag")

            attributesSeq.append("phaseDiffSpectrum")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a SysCal table
            # this is more insistant, just in case
            if hdrdom.nodeName != "SysCalTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "SysCal"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "SysCal"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("SysCal")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    SysCalRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "SysCalTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "SysCal.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "SysCal.xml")
            ...
            mimeFilePath = os.path.join(directory, "SysCal.bin")
