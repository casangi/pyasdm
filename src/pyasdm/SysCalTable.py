...
class SysCalTable:
    ...
    # A data structure to store the SysCalRow s.
    # In all cases we maintain a private list of SysCalRow s.
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x The SysCalRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a SysCalTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("SysCal is not present in memory, setting from file")
    ...
    def add(self, x):
        """
        Add a row.
        x the SysCalRow to be added.

        return a SysCalRow. If the table contains a SysCalRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added SysCalRow, otherwise x is returned.
        ...
        """
        if not isinstance(x, SysCalRow):
            raise ValueError("x must be a  SysCalRow instance.")
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of SysCalRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId, feedId):
        ...
        return a list  of SysCalRow. A None value is returned if the table contains
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "SysCalTable":
            raise ConversionException(
                "XML is not from the expected table", "SysCalTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "SysCalTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <SysCalTable")
        return
        ...
        # .append("\n")
        # .append("<SysCalTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:syscal=\"http://Alma/XASDM/SysCalTable\" xsi:schemaLocation=\"http://Alma/XASDM/SysCalTable http://almaobservatory.org/XML/XASDM/4/SysCalTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='SysCalTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<tantSpectrum/>\n")
        # .append("<tantTsysFlag/>\n")
        # .append("<tantTsysSpectrum/>\n")
        # .append("<phaseDiffFlag/>\n")
        # .append("<phaseDiffSpectrum/>\n")
        # .append("</Attributes>\n")
        # .append("</SysCalTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for SysCal")
        return
        ...
        #    for (SysCalRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "SysCal");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "SysCal");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "SysCal");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("timeInterval"); attributesSeq.add("feedId"); attributesSeq.add("numReceptor"); attributesSeq.add("numChan");
    #     attributesSeq.add("tcalFlag");  attributesSeq.add("tcalSpectrum");  attributesSeq.add("trxFlag");  attributesSeq.add("trxSpectrum");  attributesSeq.add("tskyFlag");  attributesSeq.add("tskySpectrum");  attributesSeq.add("tsysFlag");  attributesSeq.add("tsysSpectrum");  attributesSeq.add("tantFlag");  attributesSeq.add("tantSpectrum");  attributesSeq.add("tantTsysFlag");  attributesSeq.add("tantTsysSpectrum");  attributesSeq.add("phaseDiffFlag");  attributesSeq.add("phaseDiffSpectrum");
    ...
    #    else {
    #        // Then it must be a "<SysCalTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/SysCalTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/SysCalTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SysCal");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/SysCalTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SysCal");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/SysCalTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/SysCalTable/Attributes#'. Invalid XML header '"+header+"'.", "SysCal");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "SysCal");
    #    }
    #
    ...
    #    this.checkAndAdd(SysCalRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for SysCalTable")
        return

        # java code looks like this
        # File file = new File(directory+"/SysCal.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "SysCal");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "SysCal");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "SysCal found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "SysCal.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "SysCal.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/SysCal.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "SysCal");
            #
            # File binFile = new File(directory+"/SysCal.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "SysCal");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SysCal");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SysCal");
        # }
