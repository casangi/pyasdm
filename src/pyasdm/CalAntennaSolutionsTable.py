...
class CalAntennaSolutionsTable:
    ...
    Attributes:
        ...
        phaseAnt (float [] ): Array(numReceptor) the antenna based phase solution averaged over the scan (one value per receptor per antenna). See refAntennaName for the association of the values of this array with the antennas.

        phaseAntRMS (float [] ): Array(numReceptor) the RMS of the phase fluctuations relative to the antenna based average phase (one value per receptor per antenna). See refAntennaName for the association of the values of this array with the antennas.

        amplitudeAnt (float [] ): Array(numReceptor) the antenna based amplitude solution averaged over the scan (one value per receptor per antenna). See refAntennaName for the association of the values of this array with the antennas.

        amplitudeAntRMS (float [] ): Array(numReceptor) the antenna based amplitude solution averaged over the scan (one value per receptor per antenna). See refAntennaName for the association of the values of this array with the antennas.
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalAntennaSolutionsTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "CalAntennaSolutions"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("phaseAnt")

            attributesSeq.append("phaseAntRMS")

            attributesSeq.append("amplitudeAnt")

            attributesSeq.append("amplitudeAntRMS")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalAntennaSolutions table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalAntennaSolutionsTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalAntennaSolutions"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalAntennaSolutions")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalAntennaSolutionsRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalAntennaSolutionsTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalAntennaSolutions.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalAntennaSolutions.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalAntennaSolutions.bin")
