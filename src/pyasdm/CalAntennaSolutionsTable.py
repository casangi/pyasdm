...
class CalAntennaSolutionsTable:
    ...
    # A data structure to store the CalAntennaSolutionsRow s.
    # In all cases we maintain a private list of CalAntennaSolutionsRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  CalAntennaSolutionsRow s. Not needed in python.
    ...
    def __init__(self, container):
        """
        Create a CalAntennaSolutionsTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("CalAntennaSolutions is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        # the single row case
        if not isinstance(x, CalAntennaSolutionsRow):
            raise ValueError("x must be a  CalAntennaSolutionsRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new CalAntennaSolutionsRow initialized to the specified values.
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of CalAntennaSolutionsRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "CalAntennaSolutionsTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(
                        str(exc), "CalAntennaSolutionsTable"
                    ) from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalAntennaSolutionsTable")
        return
        ...
        # .append("\n")
        # .append("<CalAntennaSolutionsTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:clantsol=\"http://Alma/XASDM/CalAntennaSolutionsTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalAntennaSolutionsTable http://almaobservatory.org/XML/XASDM/4/CalAntennaSolutionsTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalAntennaSolutionsTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<phaseAntRMS/>\n")
        # .append("<amplitudeAnt/>\n")
        # .append("<amplitudeAntRMS/>\n")

        # .append("</Attributes>\n")
        # .append("</CalAntennaSolutionsTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalAntennaSolutions")
        return
        ...
        #    for (CalAntennaSolutionsRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalAntennaSolutions");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalAntennaSolutions");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalAntennaSolutions");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaName"); attributesSeq.add("atmPhaseCorrection"); attributesSeq.add("receiverBand"); attributesSeq.add("basebandName"); attributesSeq.add("calDataId"); attributesSeq.add("calReductionId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("startValidTime"); attributesSeq.add("endValidTime"); attributesSeq.add("numReceptor"); attributesSeq.add("refAntennaName"); attributesSeq.add("direction"); attributesSeq.add("frequencyRange"); attributesSeq.add("integrationTime"); attributesSeq.add("polarizationTypes"); attributesSeq.add("correctionValidity"); attributesSeq.add("phaseAnt"); attributesSeq.add("phaseAntRMS"); attributesSeq.add("amplitudeAnt"); attributesSeq.add("amplitudeAntRMS");
    #
    ...
    #    else {
    #        // Then it must be a "<CalAntennaSolutionsTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalAntennaSolutionsTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalAntennaSolutionsTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalAntennaSolutions");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalAntennaSolutionsTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalAntennaSolutions");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalAntennaSolutionsTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalAntennaSolutionsTable/Attributes#'. Invalid XML header '"+header+"'.", "CalAntennaSolutions");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalAntennaSolutions");
    #    }
    #
    ...
    #    this.checkAndAdd(CalAntennaSolutionsRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalAntennaSolutionsTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalAntennaSolutions.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalAntennaSolutions");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalAntennaSolutions");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        try:
            ...
            # reraise it as a ConversionException
            raise ConversionException(str(exc), "CalAntennaSolutionsTable") from None
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalAntennaSolutions found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CalAntennaSolutions.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CalAntennaSolutions.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/CalAntennaSolutions.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalAntennaSolutions");
            #
            # File binFile = new File(directory+"/CalAntennaSolutions.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalAntennaSolutions");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalAntennaSolutions");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalAntennaSolutions");
        # }
