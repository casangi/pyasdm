...
class DelayModelTable:
    ...
    def toXML(self):
        ...
        result += '\n<DelayModelTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:dlymdl="http://Alma/XASDM/DelayModelTable" xsi:schemaLocation="http://Alma/XASDM/DelayModelTable http://almaobservatory.org/XML/XASDM/4/DelayModelTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='DelayModelTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<numPoly/>\n"
        result += "<phaseDelay/>\n"
        result += "<phaseDelayRate/>\n"
        result += "<groupDelay/>\n"
        result += "<groupDelayRate/>\n"
        result += "<fieldId/>\n"

        result += "<timeOrigin/>\n"
        result += "<atmosphericGroupDelay/>\n"
        result += "<atmosphericGroupDelayRate/>\n"
        result += "<geometricDelay/>\n"
        result += "<geometricDelayRate/>\n"
        result += "<numLO/>\n"
        result += "<LOOffset/>\n"
        result += "<LOOffsetRate/>\n"
        result += "<dispersiveDelay/>\n"
        result += "<dispersiveDelayRate/>\n"
        result += "<atmosphericDryDelay/>\n"
        result += "<atmosphericWetDelay/>\n"
        result += "<padDelay/>\n"
        result += "<antennaDelay/>\n"
        result += "<numReceptor/>\n"
        result += "<polarizationType/>\n"
        result += "<electronicDelay/>\n"
        result += "<electronicDelayRate/>\n"
        result += "<receiverDelay/>\n"
        result += "<IFDelay/>\n"
        result += "<LODelay/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "DelayModel"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "DelayModel"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "DelayModel")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("phaseDelay")

            attributesSeq.append("phaseDelayRate")

            attributesSeq.append("groupDelay")

            attributesSeq.append("groupDelayRate")

            attributesSeq.append("fieldId")

            attributesSeq.append("timeOrigin")

            attributesSeq.append("atmosphericGroupDelay")

            attributesSeq.append("atmosphericGroupDelayRate")

            attributesSeq.append("geometricDelay")

            attributesSeq.append("geometricDelayRate")

            attributesSeq.append("numLO")

            attributesSeq.append("LOOffset")

            attributesSeq.append("LOOffsetRate")

            attributesSeq.append("dispersiveDelay")

            attributesSeq.append("dispersiveDelayRate")

            attributesSeq.append("atmosphericDryDelay")

            attributesSeq.append("atmosphericWetDelay")

            attributesSeq.append("padDelay")

            attributesSeq.append("antennaDelay")

            attributesSeq.append("numReceptor")

            attributesSeq.append("polarizationType")

            attributesSeq.append("electronicDelay")

            attributesSeq.append("electronicDelayRate")

            attributesSeq.append("receiverDelay")

            attributesSeq.append("IFDelay")
            ...
            # c++ and Java just assume it then must be a DelayModel table
            # this is more insistant, just in case
            if hdrdom.nodeName != "DelayModelTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "DelayModel"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("DelayModel")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    DelayModelRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "DelayModelTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "DelayModel.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "DelayModel.xml")
            ...
            mimeFilePath = os.path.join(directory, "DelayModel.bin")
