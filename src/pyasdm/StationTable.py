...
class StationTable:
    ...
    stationId Tag (auto-incrementable)     Station identifier. </TD>
    ...
    position  Length []   3  the position of the station.

    type  StationType  identifies the type of the station.



                 Value (Optional)

    time  ArrayTime  the time of position measurement.
    ...
    # A data structure to store the StationRow s.
    # In all cases we maintain a private list of StationRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  StationRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  stationId to Tag(nextInt).
            x.setStationId(Tag(nextInt, TagType.Station))
    ...
    def __init__(self, container):
        """
        Create a StationTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("StationTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("StationTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("Station is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "StationTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = StationRow(self)
        return thisRow

    def add(self, x):
        """
        Look up the table for a row whose noautoincrementable attributes are matching their
        homologues in x.  If a row is found that row else autoincrement x.\stationId,
        add x to its table and returns x.

        returns a StationRow.
        x. A row to be added.
        """
        if not isinstance(x, StationRow):
            raise ValueError("x must be a  StationRow instance.")

        aRow = self.lookup(x.getName(), x.getPosition(), x.getType())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, name, position, type):
        """
        Create a new StationRow initialized to the specified values.
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new StationRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new StationRow with default values for its attributes.
        """

        return StationRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if self.lookup(x.getName(), x.getPosition(), x.getType()) is not None:
            raise UniquenessViolationException(
                "Uniqueness violation exception in table StationTable"
            )

        if self.getRowByKey(x.getStationId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "StationTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of StationRow
        """
        return self._privateRows

    def getRowByKey(self, stationId):
        """
        ...
        for row in self._privateRows:
            ...
            if not row.getStationId().equals(stationId):
                continue
    ...
    def lookup(self, name, position, type):
        """
        ...
        """
        for row in self._privateRows:
            if row.compareNoAutoInc(name, position, type):
                return row
    ...
    def toXML(self):
        ...
        result += '<StationTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sttn="http://Alma/XASDM/StationTable" xsi:schemaLocation="http://Alma/XASDM/StationTable http://almaobservatory.org/XML/XASDM/4/StationTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a Station (StationTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "StationTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "StationTable":
            raise ConversionException(
                "XML is not from the expected table", "StationTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "StationTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "StationTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "StationTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "StationTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "StationTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "StationTable") from None

                except UniquenessViolationException as exc:
                    msg = (
                        "UniquenessViolationException in row in StationTable : %s"
                        % str(exc)
                    )

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "StationTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "StationTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <StationTable")
        return
        ...
        # .append("\n")
        # .append("<StationTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:sttn=\"http://Alma/XASDM/StationTable\" xsi:schemaLocation=\"http://Alma/XASDM/StationTable http://almaobservatory.org/XML/XASDM/4/StationTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='StationTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Station")
        return
        ...
        #    for (StationRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Station");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Station");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Station");
    # }
    #
    # //
    ...
    #     attributesSeq.add("stationId"); attributesSeq.add("name"); attributesSeq.add("position"); attributesSeq.add("type");
    #     attributesSeq.add("time");
    ...
    #    else {
    #        // Then it must be a "<StationTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/StationTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/StationTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Station");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/StationTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Station");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/StationTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/StationTable/Attributes#'. Invalid XML header '"+header+"'.", "Station");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Station");
    #    }
    #
    ...
    #    this.checkAndAdd(StationRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "Station.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "Station.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the Station table", "StationTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for StationTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Station.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Station");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Station");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "Station.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Station found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Station.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Station.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (Station)"
                % directory,
                "StationTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for Station")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/Station.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Station");
            #
            # File binFile = new File(directory+"/Station.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Station");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Station");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Station");
        # }
        # }
            ...
            filePath = os.path.join(directory, "Station.xml")
