...
class StationTable:
    ...
    Attributes:
        ...
        stationId (Tag): Station identifier. auto-incrementable, key.
        ...
        position (Length [] ): Array(3) the position of the station.
        ...
        time (ArrayTime): the time of position measurement. Optional.
    ...
    def toXML(self):
        ...
        result += '\n<StationTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sttn="http://Alma/XASDM/StationTable" xsi:schemaLocation="http://Alma/XASDM/StationTable http://almaobservatory.org/XML/XASDM/4/StationTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "StationTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='StationTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Station"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Station"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Station")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            # c++ and Java just assume it then must be a Station table
            # this is more insistant, just in case
            if hdrdom.nodeName != "StationTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Station"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Station"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Station")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    StationRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Station.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Station.xml")
            ...
            mimeFilePath = os.path.join(directory, "Station.bin")
