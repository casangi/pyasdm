...
class CalDelayTable:
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalDelayTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<crossDelayOffset/>\n"
        result += "<crossDelayOffsetError/>\n"
        result += "<numSideband/>\n"
        result += "<refFreq/>\n"
        result += "<refFreqPhase/>\n"
        result += "<sidebands/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalDelay"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalDelay")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("delayError")

            attributesSeq.append("delayOffset")

            attributesSeq.append("polarizationTypes")

            attributesSeq.append("reducedChiSquared")

            attributesSeq.append("appliedDelay")

            attributesSeq.append("crossDelayOffset")

            attributesSeq.append("crossDelayOffsetError")

            attributesSeq.append("numSideband")

            attributesSeq.append("refFreq")

            attributesSeq.append("refFreqPhase")

            attributesSeq.append("sidebands")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalDelay table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalDelayTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalDelay"
                )
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalDelayRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalDelayTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalDelay.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalDelay.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalDelay.bin")
