...
class ScanTable:
    ...
    def toXML(self):
        ...
        result += '\n<ScanTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:scn="http://Alma/XASDM/ScanTable" xsi:schemaLocation="http://Alma/XASDM/ScanTable http://almaobservatory.org/XML/XASDM/4/ScanTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='ScanTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<scanIntent/>\n"
        result += "<calDataType/>\n"
        result += "<calibrationOnLine/>\n"

        result += "<calibrationFunction/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Scan"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Scan"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Scan")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numIntent")

            attributesSeq.append("numSubscan")

            attributesSeq.append("scanIntent")

            attributesSeq.append("calDataType")

            attributesSeq.append("calibrationOnLine")

            attributesSeq.append("calibrationFunction")

            attributesSeq.append("calibrationSet")

            attributesSeq.append("calPattern")

            attributesSeq.append("numField")
            ...
            # c++ and Java just assume it then must be a Scan table
            # this is more insistant, just in case
            if hdrdom.nodeName != "ScanTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Scan"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Scan"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Scan")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    ScanRow.fromBin(eis, self, attributesSeq), self._checkRowUniqueness
                )
            ...
            raise ConversionException(msg, "ScanTable") from None
        except Exception as exc:
            byteStream.close()
            eis.close()
            raise ConversionException(
                "Error while reading binary data, the exception was " + str(exc), "Scan"
            ) from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Scan.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Scan.xml")
            ...
            mimeFilePath = os.path.join(directory, "Scan.bin")
