...
class FeedTable:
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        # this lookup is not done with the uniqueness check is skipped
        ...
        # this check to make sure the key is not a duplicate is only done when the uniqueness check is not skipped
        if not skipUniquenessCheck:
            if (
                self.getRowByKey(
                    x.getAntennaId(),
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "FeedTable") from None
    ...
    def setFromMIME(self, byteStream):
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Feed")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    FeedRow.fromBin(eis, self, attributesSeq), self._checkRowUniqueness
                )
