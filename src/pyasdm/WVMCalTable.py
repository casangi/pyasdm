...
class WVMCalTable:
    ...
    wvrMethod  WVRMethod  identifies the calibration method.

    polyFreqLimits  Frequency []   2  the range of frequencies in which the computation is performed.

    numInputAntenna (numInputAntenna) int  The number of antennas used for the calculations.

    numChan (numChan) int  the number of WVR channels.

    numPoly (numPoly) int  the number of coefficients used in the polynomial expansions.

    pathCoeff  float []  []  []   numInputAntenna, numChan, numPoly  the pathlengths coefficients (one value per antenna per chan per coefficient).

    refTemp  Temperature []  []   numInputAntenna, numChan  the reference temperatures (one value par antenna per channel).

    inputAntennaId  Tag []   numInputAntenna  Refers to row(s) in the Antenna table describing the antenna(s) used for the calculations. It is a 1D array expected to have numInputAntenna elements.
    ...
    # A data structure to store the WVMCalRow s.
    # In all cases we maintain a private list of WVMCalRow s.
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        """
        Insert a WVMCalRow in a list of WVMCalRow so that it's ordered by ascending start time.

        x The WVMCalRow to be inserted.
            ...
            while k0 != (k1 - 1):
                if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                    if rowlist[k0].equalByRequiredValue(x):
                        ...
                        raise DuplicateKey("DuplicateKey exception in ", "WVMCalTable")
    ...
    def __init__(self, container):
        """
        Create a WVMCalTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("WVMCalTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("WVMCalTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("WVMCal is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "WVMCalTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = WVMCalRow(self)
        return thisRow

    def add(self, x):
        """
        Add a row.
        x the WVMCalRow to be added.

        return a WVMCalRow. If the table contains a WVMCalRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added WVMCalRow, otherwise x is returned.

        raises  DuplicateKey when the table contains a WVMCalRow with a key equal to the key in x but having
         a value section different from the values in x.

         note The row is inserted in the table in such a way that all the rows having the same value of
         ( antennaId, spectralWindowId ) are stored by ascending time.
        """
        if not isinstance(x, WVMCalRow):
            raise ValueError("x must be a  WVMCalRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new WVMCalRow initialized to the specified values.
        ...
        thisRow.setNumInputAntenna(numInputAntenna)
        ...
        thisRow.setInputAntennaId(inputAntennaId)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new WVMCalRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new WVMCalRow with default values for its attributes.
        """

        return WVMCalRow(self, row)
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of WVMCalRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId):
        ...
        return a list  of WVMCalRow. A None value is returned if the table contains
    ...
    def toXML(self):
        ...
        result += '<WVMCalTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:wvmcl="http://Alma/XASDM/WVMCalTable" xsi:schemaLocation="http://Alma/XASDM/WVMCalTable http://almaobservatory.org/XML/XASDM/4/WVMCalTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a WVMCal (WVMCalTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "WVMCalTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "WVMCalTable":
            raise ConversionException(
                "XML is not from the expected table", "WVMCalTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "WVMCalTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "WVMCalTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "WVMCalTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "WVMCalTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "WVMCalTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "WVMCalTable") from None

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "WVMCalTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "WVMCalTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <WVMCalTable")
        return
        ...
        # .append("\n")
        # .append("<WVMCalTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:wvmcl=\"http://Alma/XASDM/WVMCalTable\" xsi:schemaLocation=\"http://Alma/XASDM/WVMCalTable http://almaobservatory.org/XML/XASDM/4/WVMCalTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='WVMCalTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<numInputAntenna/>\n")
        # .append("<numChan/>\n")
        # .append("<numPoly/>\n")
        # .append("<pathCoeff/>\n")
        # .append("<refTemp/>\n")
        # .append("<inputAntennaId/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for WVMCal")
        return
        ...
        #    for (WVMCalRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "WVMCal");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "WVMCal");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "WVMCal");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("timeInterval"); attributesSeq.add("wvrMethod"); attributesSeq.add("polyFreqLimits"); attributesSeq.add("numInputAntenna"); attributesSeq.add("numChan"); attributesSeq.add("numPoly"); attributesSeq.add("pathCoeff"); attributesSeq.add("refTemp"); attributesSeq.add("inputAntennaId");
    #
    ...
    #    else {
    #        // Then it must be a "<WVMCalTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/WVMCalTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/WVMCalTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "WVMCal");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/WVMCalTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "WVMCal");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/WVMCalTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/WVMCalTable/Attributes#'. Invalid XML header '"+header+"'.", "WVMCal");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "WVMCal");
    #    }
    #
    ...
    #    this.checkAndAdd(WVMCalRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "WVMCal.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "WVMCal.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the WVMCal table", "WVMCalTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for WVMCalTable")
        return

        # java code looks like this
        # File file = new File(directory+"/WVMCal.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "WVMCal");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "WVMCal");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "WVMCal.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "WVMCal found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "WVMCal.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "WVMCal.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (WVMCal)"
                % directory,
                "WVMCalTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for WVMCal")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/WVMCal.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "WVMCal");
            #
            # File binFile = new File(directory+"/WVMCal.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "WVMCal");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "WVMCal");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "WVMCal");
        # }
        # }
            ...
            filePath = os.path.join(directory, "WVMCal.xml")
