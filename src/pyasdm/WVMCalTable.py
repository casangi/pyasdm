...
class WVMCalTable(Representable):
    """
    ...
    # A list to store the WVMCalRow instances
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x is a WVMCalRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a WVMCalTable attached to container, which must be a ASDM instance
    ...
    def add(self, row):
        """
        Add a row.
        row the WVMCalRow to be added.

        return a WVMCalRow. If the table contains a WVMCalRow whose attributes (key and mandatory values) are equal to those in row
        then this returns that previously added WVMCalRow, otherwise row is returned.

        raises DuplicateKey when the table contains a WVMCalRow with a key equal to the key in row but having
    ...
    def newRow(
        ...
        """
        Create a new WVMCalRow. The new row is not added to this table, but it does know about it.
    ...
    def get(self):
        """
        Get all rows as a list of WVMCalRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId):
        ...
        """
        thisKey = WVMCalTable.Key(antennaId, spectralWindowId)
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "WVMCalTable":
            raise ConversionException(
                "XML is not from a the expected table", "WVMCalTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException("schemaVersion not found in XML", "WVMCalTable")
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "WVMCalTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("WVMCal.xml is empty", "WVMCalTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'WVMCal.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'WVMCal.xml'.
