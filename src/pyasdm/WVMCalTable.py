...
class WVMCalTable:
    ...
    Attributes:
        ...
        timeInterval (ArrayTimeInterval): the time interval for which the row's content is valid. key.
        ...
        polyFreqLimits (Frequency [] ): Array(2) the range of frequencies in which the computation is performed.

        numInputAntenna (int): The number of antennas used for the calculations.

        numChan (int): the number of WVR channels.

        numPoly (int): the number of coefficients used in the polynomial expansions.

        pathCoeff (float []  []  [] ): Array(numInputAntenna, numChan, numPoly) the pathlengths coefficients (one value per antenna per chan per coefficient).

        refTemp (Temperature []  [] ): Array(numInputAntenna, numChan) the reference temperatures (one value par antenna per channel).

        inputAntennaId (Tag [] ): Array(numInputAntenna) Refers to row(s) in the Antenna table describing the antenna(s) used for the calculations. It is a 1D array expected to have numInputAntenna elements.
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        The value of skipUniquenessCheck is not used in this version of checkAndAdd.
    ...
    def toXML(self):
        ...
        result += '\n<WVMCalTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:wvmcl="http://Alma/XASDM/WVMCalTable" xsi:schemaLocation="http://Alma/XASDM/WVMCalTable http://almaobservatory.org/XML/XASDM/4/WVMCalTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='WVMCalTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<timeInterval/>\n"
        result += "<wvrMethod/>\n"
        result += "<polyFreqLimits/>\n"
        result += "<numInputAntenna/>\n"
        result += "<numChan/>\n"
        result += "<numPoly/>\n"
        result += "<pathCoeff/>\n"
        result += "<refTemp/>\n"
        result += "<inputAntennaId/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "WVMCal"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "WVMCal"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "WVMCal")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("wvrMethod")

            attributesSeq.append("polyFreqLimits")

            attributesSeq.append("numInputAntenna")

            attributesSeq.append("numChan")

            attributesSeq.append("numPoly")

            attributesSeq.append("pathCoeff")

            attributesSeq.append("refTemp")

            attributesSeq.append("inputAntennaId")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a WVMCal table
            # this is more insistant, just in case
            if hdrdom.nodeName != "WVMCalTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "WVMCal"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "WVMCal"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("WVMCal")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    WVMCalRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "WVMCalTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "WVMCal.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "WVMCal.xml")
            ...
            mimeFilePath = os.path.join(directory, "WVMCal.bin")
