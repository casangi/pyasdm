...
class CalDataTable:
    ...
    # A data structure to store the CalDataRow s.
    # In all cases we maintain a private list of CalDataRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  CalDataRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  calDataId to Tag(nextInt).
            x.setCalDataId(Tag(nextInt, TagType.CalData))
    ...
    def __init__(self, container):
        """
        Create a CalDataTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("CalDataTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("CalDataTable")
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, CalDataRow):
            raise ValueError("x must be a  CalDataRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new CalDataRow initialized to the specified values.
        ...
        thisRow.setStartTimeObserved(startTimeObserved)

        thisRow.setEndTimeObserved(endTimeObserved)

        thisRow.setExecBlockUID(execBlockUID)

        thisRow.setCalDataType(calDataType)

        thisRow.setCalType(calType)

        thisRow.setNumScan(numScan)

        thisRow.setScanSet(scanSet)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new CalDataRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new CalDataRow with default values for its attributes.
        """

        return CalDataRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if (
            ...
            raise UniquenessViolationException(
                "Uniqueness violation exception in table CalDataTable"
            )

        if self.getRowByKey(x.getCalDataId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "CalDataTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of CalDataRow
        """
        return self._privateRows

    def getRowByKey(self, calDataId):
        """
    ...
    def toXML(self):
        ...
        result += '<CalDataTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cldata="http://Alma/XASDM/CalDataTable" xsi:schemaLocation="http://Alma/XASDM/CalDataTable http://almaobservatory.org/XML/XASDM/4/CalDataTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a CalData (CalDataTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "CalDataTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "CalDataTable":
            raise ConversionException(
                "XML is not from the expected table", "CalDataTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "CalDataTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "CalDataTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "CalDataTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "CalDataTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "CalDataTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "CalDataTable") from None

                except UniquenessViolationException as exc:
                    msg = (
                        "UniquenessViolationException in row in CalDataTable : %s"
                        % str(exc)
                    )

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "CalDataTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "CalDataTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalDataTable")
        return
        ...
        # .append("\n")
        # .append("<CalDataTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:cldata=\"http://Alma/XASDM/CalDataTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalDataTable http://almaobservatory.org/XML/XASDM/4/CalDataTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalDataTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<calDataId/>\n")
        # .append("<startTimeObserved/>\n")
        # .append("<endTimeObserved/>\n")
        # .append("<execBlockUID/>\n")
        # .append("<calDataType/>\n")
        # .append("<calType/>\n")
        # .append("<numScan/>\n")
        ...
        # .append("<assocCalDataId/>\n")
        # .append("<assocCalNature/>\n")
        # .append("<fieldName/>\n")
        # .append("<sourceName/>\n")
        # .append("<sourceCode/>\n")
        # .append("<scanIntent/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalData")
        return
        ...
        #    for (CalDataRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalData");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalData");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalData");
    # }
    #
    # //
    ...
    #     attributesSeq.add("calDataId"); attributesSeq.add("startTimeObserved"); attributesSeq.add("endTimeObserved"); attributesSeq.add("execBlockUID"); attributesSeq.add("calDataType"); attributesSeq.add("calType"); attributesSeq.add("numScan"); attributesSeq.add("scanSet");
    #     attributesSeq.add("assocCalDataId");  attributesSeq.add("assocCalNature");  attributesSeq.add("fieldName");  attributesSeq.add("sourceName");  attributesSeq.add("sourceCode");  attributesSeq.add("scanIntent");
    ...
    #    else {
    #        // Then it must be a "<CalDataTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalDataTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalDataTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalData");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalDataTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalData");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalDataTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalDataTable/Attributes#'. Invalid XML header '"+header+"'.", "CalData");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalData");
    #    }
    #
    ...
    #    this.checkAndAdd(CalDataRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "CalData.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "CalData.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the CalData table", "CalDataTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalDataTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalData.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalData");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalData");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "CalData.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalData found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CalData.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CalData.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (CalData)"
                % directory,
                "CalDataTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for CalData")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/CalData.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalData");
            #
            # File binFile = new File(directory+"/CalData.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalData");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalData");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalData");
        # }
        # }
            ...
            filePath = os.path.join(directory, "CalData.xml")
