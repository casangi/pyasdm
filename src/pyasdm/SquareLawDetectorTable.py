...
#  --------------------------------------------------------------------
#
# File SquareLawDetectorTable.py
#
...
class SquareLawDetectorTable:
    ...
    Attributes of SquareLawDetector

                 Key


    squareLawDetectorId Tag (auto-incrementable)     identifies a unique row in the table. </TD>
    ...
    numBand (numBand) int  the number of bands.

    bandType  DetectorBandType  identifies the band type.
    ...
    _tableName = "SquareLawDetector"

    # The list of field names that make up key 'key'.
    _key = ["squareLawDetectorId"]
    ...
    # A data structure to store the SquareLawDetectorRow s.
    # In all cases we maintain a private list of SquareLawDetectorRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  SquareLawDetectorRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  squareLawDetectorId to Tag(nextInt).
            x.setSquareLawDetectorId(Tag(nextInt, TagType.SquareLawDetector))
    ...
    def __init__(self, container):
        """
        Create a SquareLawDetectorTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (
                ValueError(
                    "SquareLawDetectorTable constructor must use a ASDM instance"
                )
            )
        ...
        self._entity.setEntityTypeName("SquareLawDetectorTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("SquareLawDetector is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "SquareLawDetectorTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = SquareLawDetectorRow(self)
        return thisRow

    def add(self, x):
        """
        Look up the table for a row whose noautoincrementable attributes are matching their
        homologues in x.  If a row is found that row else autoincrement x.\squareLawDetectorId,
        add x to its table and returns x.

        returns a SquareLawDetectorRow.
        x. A row to be added.
        """
        if not isinstance(x, SquareLawDetectorRow):
            raise ValueError("x must be a  SquareLawDetectorRow instance.")

        aRow = self.lookup(x.getNumBand(), x.getBandType())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, numBand, bandType):
        """
        Create a new SquareLawDetectorRow initialized to the specified values.
        ...
        thisRow.setNumBand(numBand)

        thisRow.setBandType(bandType)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new SquareLawDetectorRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new SquareLawDetectorRow with default values for its attributes.
        """

        return SquareLawDetectorRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if self.lookup(x.getNumBand(), x.getBandType()) is not None:
            raise UniquenessViolationException(
                "Uniqueness violation exception in table SquareLawDetectorTable"
            )

        if self.getRowByKey(x.getSquareLawDetectorId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "SquareLawDetectorTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of SquareLawDetectorRow
        """
        return self._privateRows

    def getRowByKey(self, squareLawDetectorId):
        """
        ...
        for row in self._privateRows:
            ...
            if not row.getSquareLawDetectorId().equals(squareLawDetectorId):
                continue
    ...
    def lookup(self, numBand, bandType):
        """
        ...
        """
        for row in self._privateRows:
            if row.compareNoAutoInc(numBand, bandType):
                return row
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a SquareLawDetector (SquareLawDetectorTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "SquareLawDetectorTable".
        if (
            not xmldom.hasChildNodes()
            or xmldom.firstChild.nodeName != "SquareLawDetectorTable"
        ):
            raise ConversionException(
                "XML is not from the expected table", "SquareLawDetectorTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "SquareLawDetectorTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "SquareLawDetectorTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "SquareLawDetectorTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "SquareLawDetectorTable"):
                    raise ConversionException(
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(
                        str(exc), "SquareLawDetectorTable"
                    ) from None

                except UniquenessViolationException as exc:
                    msg = (
                        "UniquenessViolationException in row in SquareLawDetectorTable : %s"
                        % str(exc)
                    )

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "SquareLawDetectorTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "SquareLawDetectorTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <SquareLawDetectorTable")
        return
        ...
        # .append("\n")
        # .append("<SquareLawDetectorTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:sqrlwd=\"http://Alma/XASDM/SquareLawDetectorTable\" xsi:schemaLocation=\"http://Alma/XASDM/SquareLawDetectorTable http://almaobservatory.org/XML/XASDM/4/SquareLawDetectorTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='SquareLawDetectorTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<squareLawDetectorId/>\n")
        # .append("<numBand/>\n")
        # .append("<bandType/>\n")

        # .append("</Attributes>\n")
        # .append("</SquareLawDetectorTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for SquareLawDetector")
        return
        ...
        #    for (SquareLawDetectorRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "SquareLawDetector");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "SquareLawDetector");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "SquareLawDetector");
    # }
    #
    # //
    ...
    #     attributesSeq.add("squareLawDetectorId"); attributesSeq.add("numBand"); attributesSeq.add("bandType");
    #
    ...
    #    else {
    #        // Then it must be a "<SquareLawDetectorTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/SquareLawDetectorTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/SquareLawDetectorTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SquareLawDetector");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/SquareLawDetectorTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "SquareLawDetector");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/SquareLawDetectorTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/SquareLawDetectorTable/Attributes#'. Invalid XML header '"+header+"'.", "SquareLawDetector");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "SquareLawDetector");
    #    }
    #
    ...
    #    this.checkAndAdd(SquareLawDetectorRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "SquareLawDetector.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "SquareLawDetector.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the SquareLawDetector table",
                "SquareLawDetectorTable",
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for SquareLawDetectorTable")
        return

        # java code looks like this
        # File file = new File(directory+"/SquareLawDetector.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "SquareLawDetector");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "SquareLawDetector");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "SquareLawDetector.xml")) as f:
                xmlstr = f.read()
        except Exception as exc:
            # reraise it as a ConversionException
            raise ConversionException(str(exc), "SquareLawDetectorTable") from None
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "SquareLawDetector found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "SquareLawDetector.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "SquareLawDetector.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (SquareLawDetector)"
                % directory,
                "SquareLawDetectorTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for SquareLawDetector")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/SquareLawDetector.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "SquareLawDetector");
            #
            # File binFile = new File(directory+"/SquareLawDetector.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "SquareLawDetector");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SquareLawDetector");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "SquareLawDetector");
        # }
        # }
            ...
            filePath = os.path.join(directory, "SquareLawDetector.xml")
