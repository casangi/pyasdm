...
class SquareLawDetectorTable:
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='SquareLawDetectorTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the XML header.", "SquareLawDetector"
            )
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "SquareLawDetector"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "SquareLawDetector"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("squareLawDetectorId")

            attributesSeq.append("numBand")

            attributesSeq.append("bandType")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a SquareLawDetector table
            # this is more insistant, just in case
            if hdrdom.nodeName != "SquareLawDetectorTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "SquareLawDetector"
                )
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(SquareLawDetectorRow.fromBin(eis, self, attributesSeq))
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "SquareLawDetector.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "SquareLawDetector.xml")
            ...
            mimeFilePath = os.path.join(directory, "SquareLawDetector.bin")
