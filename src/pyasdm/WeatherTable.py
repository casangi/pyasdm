...
class WeatherTable:
    ...
    Attributes:
        ...
        stationId (Tag): refers to a unique row in StationTable. key.
        ...
        pressure (Pressure): the ambient pressure. Optional.

        relHumidity (Humidity): the relative humidity. Optional.

        temperature (Temperature): the ambient temperature. Optional.

        windDirection (Angle): the wind direction. Optional.

        windSpeed (Speed): the wind speed. Optional.

        windMax (Speed): the maximum wind speed Optional.

        dewPoint (Temperature): the dew point's value. Optional.

        numLayer (int): NLayer the number of layers in the temperature profile. Optional.

        layerHeight (Length [] ): Array(numLayer) the height of each layer for the temperature profile. Optional.

        temperatureProfile (Temperature [] ): Array(numLayer) the temperature on the atmosphere at each height. Optional.

        cloudMonitor (Temperature): the temperature of the cloud monitor. Optional.

        numWVR (int): the number of WVR channels. Optional.

        wvrTemp (Temperature [] ): Array(numWVR) the observed temperature in each WVR channel. Optional.

        water (float): the water precipitable content. Optional.
    ...
    def toXML(self):
        ...
        result += '\n<WeatherTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:weathr="http://Alma/XASDM/WeatherTable" xsi:schemaLocation="http://Alma/XASDM/WeatherTable http://almaobservatory.org/XML/XASDM/4/WeatherTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='WeatherTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Weather"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Weather"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Weather")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("pressure")

            attributesSeq.append("relHumidity")

            attributesSeq.append("temperature")

            attributesSeq.append("windDirection")

            attributesSeq.append("windSpeed")

            attributesSeq.append("windMax")

            attributesSeq.append("dewPoint")

            attributesSeq.append("numLayer")

            attributesSeq.append("layerHeight")

            attributesSeq.append("temperatureProfile")

            attributesSeq.append("cloudMonitor")

            attributesSeq.append("numWVR")

            attributesSeq.append("wvrTemp")

            attributesSeq.append("water")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a Weather table
            # this is more insistant, just in case
            if hdrdom.nodeName != "WeatherTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Weather"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "Weather"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Weather")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    WeatherRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "WeatherTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Weather.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Weather.xml")
            ...
            mimeFilePath = os.path.join(directory, "Weather.bin")
