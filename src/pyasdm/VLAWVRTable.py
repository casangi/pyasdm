...
class VLAWVRTable:
    ...
    # A data structure to store the VLAWVRRow s.
    # In all cases we maintain a private list of VLAWVRRow s.
    _privateRows = []
    ...
    # The tolerance which will be used on chanFreqCenter during an add operation on the table
    _chanFreqCenterEqTolerance = Frequency(0.0)

    def setChanFreqCenterEqTolerance(self, tolerance):
        """
        A setter for the tolerance on chanFreqCenter
        """
        if not isinstance(tolerance, Frequency):
            print("tolerance must be a  Frequency instance")

        self._chanFreqCenterEqTolerance = Frequency(tolerance)

    def getChanFreqCenterEqTolerance(self):
        """
        A getter for the tolerance on chanFreqCenter
        Returns the tolerance as a  Frequency
        """
        return self._chanFreqCenterEqTolerance
    ...
    def insertByStartTime(self, x, rowlist):
        """
        Insert a VLAWVRRow in a list of VLAWVRRow so that it's ordered by ascending start time.

        x The VLAWVRRow to be inserted.
            ...
            while k0 != (k1 - 1):
                if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                    if rowlist[k0].equalByRequiredValue(x):
                        ...
                        raise DuplicateKey("DuplicateKey exception in ", "VLAWVRTable")
    ...
    def __init__(self, container):
        """
        Create a VLAWVRTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (ValueError("VLAWVRTable constructor must use a ASDM instance"))
        ...
        self._entity.setEntityTypeName("VLAWVRTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("VLAWVR is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "VLAWVRTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = VLAWVRRow(self)
        return thisRow

    def add(self, x):
        """
        Add a row.
        x the VLAWVRRow to be added.

        return a VLAWVRRow. If the table contains a VLAWVRRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added VLAWVRRow, otherwise x is returned.

        raises  DuplicateKey when the table contains a VLAWVRRow with a key equal to the key in x but having
         a value section different from the values in x.

         note The row is inserted in the table in such a way that all the rows having the same value of
         ( antennaId ) are stored by ascending time.
        """
        if not isinstance(x, VLAWVRRow):
            raise ValueError("x must be a  VLAWVRRow instance.")
    ...
    def newRow(self, antennaId, timeInterval, numChan, hiValues, loValues):
        """
        Create a new VLAWVRRow initialized to the specified values.
        ...
        thisRow.setHiValues(hiValues)

        thisRow.setLoValues(loValues)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new VLAWVRRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new VLAWVRRow with default values for its attributes.
        """

        return VLAWVRRow(self, row)
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of VLAWVRRow
        """
    ...
    def getByContext(self, antennaId):
        ...
        return a list  of VLAWVRRow. A None value is returned if the table contains
    ...
    def toXML(self):
        ...
        result += '<VLAWVRTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:vlawvr="http://Alma/XASDM/VLAWVRTable" xsi:schemaLocation="http://Alma/XASDM/VLAWVRTable http://almaobservatory.org/XML/XASDM/4/VLAWVRTable.xsd" schemaVersion="4" schemaRevision="-1">\n'
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a VLAWVR (VLAWVRTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "VLAWVRTable".
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "VLAWVRTable":
            raise ConversionException(
                "XML is not from the expected table", "VLAWVRTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "VLAWVRTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "VLAWVRTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "VLAWVRTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "VLAWVRTable"):
                    raise ConversionException(
                ...
                if hasContainerEntity:
                    raise ConversionException(
                        "More than one ContainerEntity found in XML", "VLAWVRTable"
                    )
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "VLAWVRTable") from None

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "VLAWVRTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "VLAWVRTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <VLAWVRTable")
        return
        ...
        # .append("\n")
        # .append("<VLAWVRTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:vlawvr=\"http://Alma/XASDM/VLAWVRTable\" xsi:schemaLocation=\"http://Alma/XASDM/VLAWVRTable http://almaobservatory.org/XML/XASDM/4/VLAWVRTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='VLAWVRTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for VLAWVR")
        return
        ...
        #    for (VLAWVRRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "VLAWVR");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "VLAWVR");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "VLAWVR");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("timeInterval"); attributesSeq.add("numChan"); attributesSeq.add("hiValues"); attributesSeq.add("loValues");
    #     attributesSeq.add("chanFreqCenter");  attributesSeq.add("chanWidth");  attributesSeq.add("wvrId");
    ...
    #    else {
    #        // Then it must be a "<VLAWVRTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/VLAWVRTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/VLAWVRTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "VLAWVR");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/VLAWVRTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "VLAWVR");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/VLAWVRTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/VLAWVRTable/Attributes#'. Invalid XML header '"+header+"'.", "VLAWVR");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "VLAWVR");
    #    }
    #
    ...
    #    this.checkAndAdd(VLAWVRRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "VLAWVR.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "VLAWVR.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the VLAWVR table", "VLAWVRTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for VLAWVRTable")
        return

        # java code looks like this
        # File file = new File(directory+"/VLAWVR.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "VLAWVR");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "VLAWVR");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "VLAWVR.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "VLAWVR found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "VLAWVR.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "VLAWVR.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (VLAWVR)"
                % directory,
                "VLAWVRTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for VLAWVR")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/VLAWVR.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "VLAWVR");
            #
            # File binFile = new File(directory+"/VLAWVR.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "VLAWVR");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "VLAWVR");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "VLAWVR");
        # }
        # }
            ...
            filePath = os.path.join(directory, "VLAWVR.xml")
