...
class VLAWVRTable(Representable):
    """
    ...
    # A list to store the VLAWVRRow instances
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x is a VLAWVRRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a VLAWVRTable attached to container, which must be a ASDM instance
    ...
    def add(self, row):
        """
        Add a row.
        row the VLAWVRRow to be added.

        return a VLAWVRRow. If the table contains a VLAWVRRow whose attributes (key and mandatory values) are equal to those in row
        then this returns that previously added VLAWVRRow, otherwise row is returned.

        raises DuplicateKey when the table contains a VLAWVRRow with a key equal to the key in row but having
    ...
    def newRow(self, antennaId, timeInterval, numChan, hiValues, loValues):
        """
        Create a new VLAWVRRow. The new row is not added to this table, but it does know about it.
    ...
    def get(self):
        """
        Get all rows as a list of VLAWVRRow
        """
    ...
    def getByContext(self, antennaId):
        ...
        ( antennaId ).
        """
        thisKey = VLAWVRTable.Key(antennaId)
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "VLAWVRTable":
            raise ConversionException(
                "XML is not from a the expected table", "VLAWVRTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException("schemaVersion not found in XML", "VLAWVRTable")
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "VLAWVRTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("VLAWVR.xml is empty", "VLAWVRTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'VLAWVR.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'VLAWVR.xml'.
