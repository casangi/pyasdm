...
class VLAWVRTable:
    ...
    def toXML(self):
        ...
        result += '\n<VLAWVRTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:vlawvr="http://Alma/XASDM/VLAWVRTable" xsi:schemaLocation="http://Alma/XASDM/VLAWVRTable http://almaobservatory.org/XML/XASDM/4/VLAWVRTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='VLAWVRTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "VLAWVR"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "VLAWVR"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "VLAWVR")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("hiValues")

            attributesSeq.append("loValues")

            attributesSeq.append("chanFreqCenter")

            attributesSeq.append("chanWidth")

            attributesSeq.append("wvrId")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a VLAWVR table
            # this is more insistant, just in case
            if hdrdom.nodeName != "VLAWVRTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "VLAWVR"
                )
            ...
            if not hdrdom.hasChildNodes():
                byteStream.close()
                raise ConversionException(
                    "THe XML header is missing all of the expected elements.", "VLAWVR"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("VLAWVR")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    VLAWVRRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "VLAWVRTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "VLAWVR.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "VLAWVR.xml")
            ...
            mimeFilePath = os.path.join(directory, "VLAWVR.bin")
