...
class AlmaRadiometerTable:
    ...
    almaRadiometerId Tag (auto-incrementable)     identifies a unique row in the table. </TD>
    ...
    # A data structure to store the AlmaRadiometerRow s.
    # In all cases we maintain a private list of AlmaRadiometerRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  AlmaRadiometerRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  almaRadiometerId to Tag(nextInt).
            x.setAlmaRadiometerId(Tag(nextInt, TagType.AlmaRadiometer))
    ...
    def __init__(self, container):
        """
        Create a AlmaRadiometerTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("AlmaRadiometer is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        homologues in x.  If a row is found that row else autoincrement x.\almaRadiometerId,
        add x to its table and returns x.

        returns a AlmaRadiometerRow.
        x. A row to be added.
        """
        if not isinstance(x, AlmaRadiometerRow):
            raise ValueError("x must be a  AlmaRadiometerRow instance.")
    ...
    def checkAndAdd(self, x):
        ...
        if self.getRowByKey(x.getAlmaRadiometerId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of AlmaRadiometerRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "AlmaRadiometerTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "AlmaRadiometerTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <AlmaRadiometerTable")
        return
        ...
        # .append("\n")
        # .append("<AlmaRadiometerTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:ardmtr=\"http://Alma/XASDM/AlmaRadiometerTable\" xsi:schemaLocation=\"http://Alma/XASDM/AlmaRadiometerTable http://almaobservatory.org/XML/XASDM/4/AlmaRadiometerTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='AlmaRadiometerTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<almaRadiometerId/>\n")

        # .append("<numAntenna/>\n")
        # .append("<spectralWindowId/>\n")
        # .append("</Attributes>\n")
        # .append("</AlmaRadiometerTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for AlmaRadiometer")
        return
        ...
        #    for (AlmaRadiometerRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "AlmaRadiometer");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "AlmaRadiometer");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "AlmaRadiometer");
    # }
    #
    # //
    ...
    #     attributesSeq.add("almaRadiometerId");
    #     attributesSeq.add("numAntenna");  attributesSeq.add("spectralWindowId");
    ...
    #    else {
    #        // Then it must be a "<AlmaRadiometerTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/AlmaRadiometerTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/AlmaRadiometerTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "AlmaRadiometer");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/AlmaRadiometerTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "AlmaRadiometer");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/AlmaRadiometerTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/AlmaRadiometerTable/Attributes#'. Invalid XML header '"+header+"'.", "AlmaRadiometer");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "AlmaRadiometer");
    #    }
    #
    ...
    #    this.checkAndAdd(AlmaRadiometerRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for AlmaRadiometerTable")
        return

        # java code looks like this
        # File file = new File(directory+"/AlmaRadiometer.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "AlmaRadiometer");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "AlmaRadiometer");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "AlmaRadiometer found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "AlmaRadiometer.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "AlmaRadiometer.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/AlmaRadiometer.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "AlmaRadiometer");
            #
            # File binFile = new File(directory+"/AlmaRadiometer.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "AlmaRadiometer");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "AlmaRadiometer");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "AlmaRadiometer");
        # }
