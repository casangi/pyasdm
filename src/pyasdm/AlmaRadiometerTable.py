...
class AlmaRadiometerTable:
    ...
    Attributes:
        ...
        almaRadiometerId (Tag): identifies a unique row in the table. auto-incrementable, key.
        ...
        numAntenna (int): the number of antennas. Optional.

        spectralWindowId (Tag [] ): Array(numAntenna) the references to the actual spectral windows (one spectral window per antenna). Optional.
    ...
    def toXML(self):
        ...
        result += '\n<AlmaRadiometerTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ardmtr="http://Alma/XASDM/AlmaRadiometerTable" xsi:schemaLocation="http://Alma/XASDM/AlmaRadiometerTable http://almaobservatory.org/XML/XASDM/4/AlmaRadiometerTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "AlmaRadiometerTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='AlmaRadiometerTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "AlmaRadiometer"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "AlmaRadiometer"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "AlmaRadiometer"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("almaRadiometerId")
            ...
            # c++ and Java just assume it then must be a AlmaRadiometer table
            # this is more insistant, just in case
            if hdrdom.nodeName != "AlmaRadiometerTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "AlmaRadiometer"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("AlmaRadiometer")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    AlmaRadiometerRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "AlmaRadiometer.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "AlmaRadiometer.xml")
            ...
            mimeFilePath = os.path.join(directory, "AlmaRadiometer.bin")
