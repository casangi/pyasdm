...
class DataDescriptionTable:
    ...
    dataDescriptionId Tag (auto-incrementable)     identifies a unique row in the table. </TD>
    ...
    # In all cases we maintain a private list of DataDescriptionRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  DataDescriptionRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  dataDescriptionId to Tag(nextInt).
            x.setDataDescriptionId(Tag(nextInt, TagType.DataDescription))
    ...
    def __init__(self, container):
        """
        Create a DataDescriptionTable attached to container.
    ...
    def add(self, x):
        ...
        homologues in x.  If a row is found that row else autoincrement x.\dataDescriptionId,
        add x to its table and returns x.

        returns a DataDescriptionRow.
        x. A row to be added.
        """
        if not isinstance(x, DataDescriptionRow):
            raise ValueError("x must be a  DataDescriptionRow instance.")

        aRow = self.lookup(x.getPolOrHoloId(), x.getSpectralWindowId())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, polOrHoloId, spectralWindowId):
        """
        Create a new DataDescriptionRow initialized to the specified values.
    ...
    def checkAndAdd(self, x):
        ...
        if self.lookup(x.getPolOrHoloId(), x.getSpectralWindowId()) is not None:
            raise UniquenessViolationException(
                "Uniqueness violation exception in table DataDescriptionTable"
            )

        if self.getRowByKey(x.getDataDescriptionId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of DataDescriptionRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "DataDescriptionTable"
            )
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <DataDescriptionTable")
        return
        ...
        # .append("\n")
        # .append("<DataDescriptionTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:dtdsc=\"http://Alma/XASDM/DataDescriptionTable\" xsi:schemaLocation=\"http://Alma/XASDM/DataDescriptionTable http://almaobservatory.org/XML/XASDM/4/DataDescriptionTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='DataDescriptionTable' schemaVersion='1' documentVersion='1'/>\n")
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for DataDescription")
        return
        ...
        #    for (DataDescriptionRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "DataDescription");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "DataDescription");
    #
    ...
    #     attributesSeq.add("dataDescriptionId"); attributesSeq.add("polOrHoloId"); attributesSeq.add("spectralWindowId");
    #     attributesSeq.add("pulsarId");
    ...
    #    else {
    #        // Then it must be a "<DataDescriptionTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/DataDescriptionTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/DataDescriptionTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "DataDescription");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/DataDescriptionTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "DataDescription");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/DataDescriptionTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/DataDescriptionTable/Attributes#'. Invalid XML header '"+header+"'.", "DataDescription");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "DataDescription");
    #    }
    #
    ...
    #    this.checkAndAdd(DataDescriptionRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for DataDescriptionTable")
        return

        # java code looks like this
        # File file = new File(directory+"/DataDescription.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "DataDescription");
        #
    ...
    def setFromXMLFile(self, directory):
        ...
        try:
            ...
            # reraise it as a ConversionException
            raise ConversionException(str(exc), "DataDescriptionTable") from None
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "DataDescription found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "DataDescription.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "DataDescription.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/DataDescription.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "DataDescription");
            #
            # File binFile = new File(directory+"/DataDescription.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "DataDescription");
            #
            # try {
