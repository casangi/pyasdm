...
class DataDescriptionTable:
    ...
    Attributes:
        ...
        dataDescriptionId (Tag): identifies a unique row in the table. auto-incrementable, key.
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        if not skipUniquenessCheck:
            if self.getRowByKey(x.getDataDescriptionId()) is not None:
                raise DuplicateKey(
                    "Duplicate key exception in ", "DataDescriptionTable"
                )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "DataDescriptionTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='DataDescriptionTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the XML header.", "DataDescription"
            )
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "DataDescription"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "DataDescription"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("polOrHoloId")
            ...
            # c++ and Java just assume it then must be a DataDescription table
            # this is more insistant, just in case
            if hdrdom.nodeName != "DataDescriptionTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "DataDescription"
                )
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    DataDescriptionRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "DataDescription.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "DataDescription.xml")
            ...
            mimeFilePath = os.path.join(directory, "DataDescription.bin")
