...
class FocusModelTable:
    ...
    def toXML(self):
        ...
        result += '\n<FocusModelTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:focsm="http://Alma/XASDM/FocusModelTable" xsi:schemaLocation="http://Alma/XASDM/FocusModelTable http://almaobservatory.org/XML/XASDM/4/FocusModelTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "FocusModelTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='FocusModelTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "FocusModel"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "FocusModel"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "FocusModel")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("assocFocusModelId")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a FocusModel table
            # this is more insistant, just in case
            if hdrdom.nodeName != "FocusModelTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "FocusModel"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("FocusModel")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    FocusModelRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "FocusModel.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "FocusModel.xml")
            ...
            mimeFilePath = os.path.join(directory, "FocusModel.bin")
