...
class FocusTable:
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <FocusTable")
        return
        ...
        # .append("\n")
        # .append("<FocusTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:focus=\"http://Alma/XASDM/FocusTable\" xsi:schemaLocation=\"http://Alma/XASDM/FocusTable http://almaobservatory.org/XML/XASDM/4/FocusTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='FocusTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<antennaId/>\n")
        # .append("<timeInterval/>\n")
        # .append("<focusTracking/>\n")
        # .append("<focusOffset/>\n")
        # .append("<focusRotationOffset/>\n")
        # .append("<focusModelId/>\n")

        # .append("<measuredFocusPosition/>\n")
        # .append("<measuredFocusRotation/>\n")
        # .append("</Attributes>\n")
        # .append("</FocusTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for Focus")
        return
        ...
        #    for (FocusRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "Focus");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "Focus");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "Focus");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("timeInterval"); attributesSeq.add("focusTracking"); attributesSeq.add("focusOffset"); attributesSeq.add("focusRotationOffset"); attributesSeq.add("focusModelId");
    #     attributesSeq.add("measuredFocusPosition");  attributesSeq.add("measuredFocusRotation");
    ...
    #    else {
    #        // Then it must be a "<FocusTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/FocusTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/FocusTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Focus");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/FocusTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "Focus");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/FocusTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/FocusTable/Attributes#'. Invalid XML header '"+header+"'.", "Focus");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "Focus");
    #    }
    #
    ...
    #    this.checkAndAdd(FocusRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for FocusTable")
        return

        # java code looks like this
        # File file = new File(directory+"/Focus.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "Focus");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "Focus");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "Focus found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for Focus")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/Focus.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "Focus");
            #
            # File binFile = new File(directory+"/Focus.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "Focus");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Focus");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "Focus");
        # }
