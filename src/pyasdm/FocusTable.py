...
class FocusTable(Representable):
    """
    ...
    # A list to store the FocusRow instances
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x is a FocusRow to be inserted.
    ...
    def __init__(self, container):
        """
        Create a FocusTable attached to container, which must be a ASDM instance
    ...
    def add(self, row):
        """
        Add a row.
        row the FocusRow to be added.

        return a FocusRow. If the table contains a FocusRow whose attributes (key and mandatory values) are equal to those in row
        then this returns that previously added FocusRow, otherwise row is returned.

        raises DuplicateKey when the table contains a FocusRow with a key equal to the key in row but having
    ...
    def newRow(
        ...
        """
        Create a new FocusRow. The new row is not added to this table, but it does know about it.
    ...
    def get(self):
        """
        Get all rows as a list of FocusRow
        """
    ...
    def getByContext(self, antennaId):
        ...
        ( antennaId ).
        """
        thisKey = FocusTable.Key(antennaId)
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "FocusTable":
            raise ConversionException(
                "XML is not from a the expected table", "FocusTable."
            )
        ...
        if (not tabdom.hasAttributes()) or (
            ...
            raise ConversionException("schemaVersion not found in XML", "FocusTable")
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str, "FocusTable") from None
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr is None:
            raise ConversionException("Focus.xml is empty", "FocusTable")
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin=True) will be saved in a file 'Focus.bin' or an
        XML representation (_fileAsBin==False) will be saved in a file 'Focus.xml'.
