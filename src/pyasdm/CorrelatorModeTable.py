...
class CorrelatorModeTable:
    ...
    _tableName = "CorrelatorMode"

    # The list of field names that make up key 'key'.
    _key = ["correlatorModeId"]
    ...
    # A data structure to store the CorrelatorModeRow s.
    # In all cases we maintain a private list of CorrelatorModeRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  CorrelatorModeRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  correlatorModeId to Tag(nextInt).
            x.setCorrelatorModeId(Tag(nextInt, TagType.CorrelatorMode))
    ...
    def __init__(self, container):
        """
        Create a CorrelatorModeTable attached to container.
        ...
        if not isinstance(container, pyasdm.ASDM):
            raise (
                ValueError("CorrelatorModeTable constructor must use a ASDM instance")
            )
        ...
        self._entity.setEntityTypeName("CorrelatorModeTable")
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("CorrelatorMode is not present in memory, setting from file")
    ...
    def toString(self):
        """
        Returns "CorrelatorModeTable" followed by the current size of the table
        between parenthesis.
    ...
    def newRowDefault(self):
        ...
        """
        thisRow = CorrelatorModeRow(self)
        return thisRow
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, CorrelatorModeRow):
            raise ValueError("x must be a  CorrelatorModeRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new CorrelatorModeRow initialized to the specified values.
        ...
        thisRow.setNumBaseband(numBaseband)

        thisRow.setBasebandNames(basebandNames)

        thisRow.setBasebandConfig(basebandConfig)

        thisRow.setAccumMode(accumMode)

        thisRow.setBinMode(binMode)

        thisRow.setNumAxes(numAxes)

        thisRow.setAxesOrderArray(axesOrderArray)

        thisRow.setFilterMode(filterMode)

        thisRow.setCorrelatorName(correlatorName)
    ...
    def newRowCopy(self, row):
        ...
        The method creates a new CorrelatorModeRow which knows about this table.
        Each attribute of the created row is a (deep) copy of the corresponding
        attribute of row. The method does not add the created row to this,
        it simply parents it to this, a call to the add method
        has to be done in order to get the row added (very likely after having modified
        some of its attributes.
        If row is None then the method returns a new CorrelatorModeRow with default values for its attributes.
        """

        return CorrelatorModeRow(self, row)
    ...
    def checkAndAdd(self, x):
        ...
        if (
            ...
            raise UniquenessViolationException(
                "Uniqueness violation exception in table CorrelatorModeTable"
            )

        if self.getRowByKey(x.getCorrelatorModeId()) is not None:
            raise DuplicateKey("Duplicate key exception in ", "CorrelatorModeTable")
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of CorrelatorModeRow
        """
        return self._privateRows

    def getRowByKey(self, correlatorModeId):
        """
        ...
        for row in self._privateRows:
            ...
            if not row.getCorrelatorModeId().equals(correlatorModeId):
                continue
    ...
    def fromXML(self, xmlstr):
        ...
        conform to the XML schema defined for a CorrelatorMode (CorrelatorModeTable.xsd).
        """
        if not isinstance(xmlstr, str):
            raise ConversionException("xmlstr must be a string")

        xmldom = minidom.parseString(xmlstr)
        # this should have at least one child node with a name of "CorrelatorModeTable".
        if (
            not xmldom.hasChildNodes()
            or xmldom.firstChild.nodeName != "CorrelatorModeTable"
        ):
            raise ConversionException(
                "XML is not from the expected table", "CorrelatorModeTable"
            )
        ...
        try:
            ...
            raise ConversionException(
                "schemaVersion is not an integer", "CorrelatorModeTable"
            ) from None
        ...
        if not tabdom.hasChildNodes():
            raise ConversionException(
                "XML is missing all of the expected elements", "CorrelatorModeTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                if tabEntity is not None:
                    raise ConversionException(
                        "More than one Entity found in XML", "CorrelatorModeTable"
                    )
                tabEntity = Entity(thisNode.toxml())
                if not (tabEntity.getEntityTypeName() == "CorrelatorModeTable"):
                    raise ConversionException(
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "CorrelatorModeTable") from None

                except UniquenessViolationException as exc:
                    msg = (
                        "UniquenessViolationException in row in CorrelatorModeTable : %s"
                        % str(exc)
                    )

        if tabEntity is None:
            raise ConversionException("No Entity seen in XML", "CorrelatorModeTable")
        if not hasContainerEntity:
            raise ValueError("No Container Entity seen in XL", "CorrelatorModeTable")

        self.setEntity(tabEntity)

    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CorrelatorModeTable")
        return
        ...
        # .append("\n")
        # .append("<CorrelatorModeTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:cormod=\"http://Alma/XASDM/CorrelatorModeTable\" xsi:schemaLocation=\"http://Alma/XASDM/CorrelatorModeTable http://almaobservatory.org/XML/XASDM/4/CorrelatorModeTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CorrelatorModeTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<correlatorModeId/>\n")
        # .append("<numBaseband/>\n")
        # .append("<basebandNames/>\n")
        # .append("<basebandConfig/>\n")
        # .append("<accumMode/>\n")
        # .append("<binMode/>\n")
        # .append("<numAxes/>\n")
        # .append("<axesOrderArray/>\n")
        # .append("<filterMode/>\n")
        # .append("<correlatorName/>\n")

        # .append("</Attributes>\n")
        # .append("</CorrelatorModeTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CorrelatorMode")
        return
        ...
        #    for (CorrelatorModeRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CorrelatorMode");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CorrelatorMode");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CorrelatorMode");
    # }
    #
    # //
    ...
    #     attributesSeq.add("correlatorModeId"); attributesSeq.add("numBaseband"); attributesSeq.add("basebandNames"); attributesSeq.add("basebandConfig"); attributesSeq.add("accumMode"); attributesSeq.add("binMode"); attributesSeq.add("numAxes"); attributesSeq.add("axesOrderArray"); attributesSeq.add("filterMode"); attributesSeq.add("correlatorName");
    #
    ...
    #    else {
    #        // Then it must be a "<CorrelatorModeTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CorrelatorModeTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CorrelatorModeTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CorrelatorMode");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CorrelatorModeTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CorrelatorMode");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CorrelatorModeTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CorrelatorModeTable/Attributes#'. Invalid XML header '"+header+"'.", "CorrelatorMode");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CorrelatorMode");
    #    }
    #
    ...
    #    this.checkAndAdd(CorrelatorModeRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromFile(self, directory):
        ...
        if os.path.exists(os.path.join(directory, "CorrelatorMode.xml")):
            self.setFromXMLFile(directory)
        elif os.path.exists(os.path.join(directory, "CorrelatorMode.bin")):
            self.setFromMIMEFile(directory)
        else:
            raise ConversionException(
                "No file found for the CorrelatorMode table", "CorrelatorModeTable"
            )
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CorrelatorModeTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CorrelatorMode.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CorrelatorMode");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CorrelatorMode");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        xmlstr = None
        try:
            with open(os.path.join(directory, "CorrelatorMode.xml")) as f:
                xmlstr = f.read()
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CorrelatorMode found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CorrelatorMode.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CorrelatorMode.xml".
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise ConversionException(
                "Cannot write into directory %s. This file already exists and is not a directory. (CorrelatorMode)"
                % directory,
                "CorrelatorModeTable",
            )
        ...
        if self._fileAsBin:
            print("fileAsBin not yet implemented for CorrelatorMode")
            # the Java code looks like this
            #
            # The table is exported in a binary format.
            # (actually a short XML file + a possibly long MIME file)
            #
            # File xmlFile = new File(directory+"/CorrelatorMode.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CorrelatorMode");
            #
            # File binFile = new File(directory+"/CorrelatorMode.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CorrelatorMode");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CorrelatorMode");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CorrelatorMode");
        # }
        # }
            ...
            filePath = os.path.join(directory, "CorrelatorMode.xml")
