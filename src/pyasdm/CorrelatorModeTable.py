...
class CorrelatorModeTable:
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    raise ConversionException(msg, "CorrelatorModeTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CorrelatorModeTable' schemaVersion='1' documentVersion='1'/>\n"
        ...
        result += "<correlatorModeId/>\n"
        result += "<numBaseband/>\n"
        result += "<basebandNames/>\n"
        result += "<basebandConfig/>\n"
        result += "<accumMode/>\n"
        result += "<binMode/>\n"
        result += "<numAxes/>\n"
        result += "<axesOrderArray/>\n"
        result += "<filterMode/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CorrelatorMode"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CorrelatorMode"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException(
                "XML is not properly structured.", "CorrelatorMode"
            )
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("correlatorModeId")

            attributesSeq.append("numBaseband")

            attributesSeq.append("basebandNames")

            attributesSeq.append("basebandConfig")

            attributesSeq.append("accumMode")

            attributesSeq.append("binMode")

            attributesSeq.append("numAxes")

            attributesSeq.append("axesOrderArray")
            ...
            # c++ and Java just assume it then must be a CorrelatorMode table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CorrelatorModeTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CorrelatorMode"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CorrelatorMode")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CorrelatorModeRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CorrelatorMode.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CorrelatorMode.xml")
            ...
            mimeFilePath = os.path.join(directory, "CorrelatorMode.bin")
