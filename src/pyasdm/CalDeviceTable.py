...
class CalDeviceTable:
    ...
    # A data structure to store the CalDeviceRow s.
    # In all cases we maintain a private list of CalDeviceRow s.
    _privateRows = []
    ...
    def insertByStartTime(self, x, rowlist):
        ...
        x The CalDeviceRow to be inserted.
            ...
            while k0 != (k1 - 1):
                if xTimeStart == rowlist[k0].getTimeInterval().getStart().get():
                    if rowlist[k0].equalByRequiredValue(x):
                        ...
                        raise DuplicateKey(
                            "DuplicateKey exception in ", "CalDeviceTable"
                        )
    ...
    def __init__(self, container):
        """
        Create a CalDeviceTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            print("CalDevice is not present in memory, setting from file")
    ...
    def add(self, x):
        """
        Add a row.
        x the CalDeviceRow to be added.

        return a CalDeviceRow. If the table contains a CalDeviceRow whose attributes (key and mandatory values) are equal to this in x
        then this returns that previously added CalDeviceRow, otherwise x is returned.

        raises  DuplicateKey when the table contains a CalDeviceRow with a key equal to the key in x but having
        ...
        """
        if not isinstance(x, CalDeviceRow):
            raise ValueError("x must be a  CalDeviceRow instance.")
    ...
    def newRow(
        ...
        """
        Create a new CalDeviceRow initialized to the specified values.
    ...
    def get(self):
        """
        Get all rows.
        return all rows as a list of CalDeviceRow
        """
    ...
    def getByContext(self, antennaId, spectralWindowId, feedId):
        ...
        return a list  of CalDeviceRow. A None value is returned if the table contains
    ...
    def fromXML(self, xmlstr):
        ...
        if not xmldom.hasChildNodes() or xmldom.firstChild.nodeName != "CalDeviceTable":
            raise ConversionException(
                "XML is not from the expected table", "CalDeviceTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "CalDeviceTable") from None
    ...
    def MIMEXMLPart(self):
        print("MIMEXMLPart not implemented for <CalDeviceTable")
        return
        ...
        # .append("\n")
        # .append("<CalDeviceTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:cldvc=\"http://Alma/XASDM/CalDeviceTable\" xsi:schemaLocation=\"http://Alma/XASDM/CalDeviceTable http://almaobservatory.org/XML/XASDM/4/CalDeviceTable.xsd\" schemaVersion=\"4\" schemaRevision=\"-1\">\n")
        # .append("<Entity entityId='")
        # .append(UID)
        # .append("' entityIdEncrypted='na' entityTypeName='CalDeviceTable' schemaVersion='1' documentVersion='1'/>\n")
        ...
        # .append("<calEff/>\n")
        # .append("<noiseCal/>\n")
        # .append("<coupledNoiseCal/>\n")
        # .append("<temperatureLoad/>\n")
        # .append("</Attributes>\n")
        # .append("</CalDeviceTable>\n");
        # return sb.toString();
    ...
    def toMIME(self):
        ...
        """
        print("toMIME not yet implemented for CalDevice")
        return
        ...
        #    for (CalDeviceRow row: privateRows) row.toBin(dos);
    ...
    #    if (loc0 == -1 ) throw new ConversionException("Failed to detect the beginning of the XML header", "CalDevice");
    #
    #    loc0 += xmlPartMIMEHeader.length();
    #
    #    // Look for the string announcing the binary part.
    #    int loc1 = s.indexOf(binPartMIMEHeader, loc0);
    #    if (loc1 == -1) throw new ConversionException("Failed to detect the beginning of the binary part", "CalDevice");
    #
    ...
    # }
    # catch (Exception e) {
    #     throw new ConversionException(e.getMessage(), "CalDevice");
    # }
    #
    # //
    ...
    #     attributesSeq.add("antennaId"); attributesSeq.add("spectralWindowId"); attributesSeq.add("timeInterval"); attributesSeq.add("feedId"); attributesSeq.add("numCalload"); attributesSeq.add("calLoadNames");
    #     attributesSeq.add("numReceptor");  attributesSeq.add("calEff");  attributesSeq.add("noiseCal");  attributesSeq.add("coupledNoiseCal");  attributesSeq.add("temperatureLoad");
    ...
    #    else {
    #        // Then it must be a "<CalDeviceTable ...." document
    #        // With a BulkStoreRef child element....
    #        XPath xpa = XPath.newInstance("/CalDeviceTable/BulkStoreRef/@byteOrder");
    #        Object node = xpa.selectSingleNode(document.getRootElement());
    #        if (node == null)
    #            throw new ConversionException("No element found for the XPath expression '/CalDeviceTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalDevice");
    #
    ...
    #            byteOrder = ByteOrder.BIG_ENDIAN;
    #        else
    #            throw new ConversionException("No valid value retrieved for the node '/CalDeviceTable/BulkStoreRef/@byteOrder'. Invalid XML header '"+header+"'.", "CalDevice");
    #
    #        // And also it must have an Attributes element with children.
    #        xpa = XPath.newInstance("/CalDeviceTable/Attributes#");
    #        List nodes = xpa.selectNodes(document.getRootElement());
    #        if (nodes==null || nodes.size()==0)
    #            throw new ConversionException("No element found for the XPath expression '/CalDeviceTable/Attributes#'. Invalid XML header '"+header+"'.", "CalDevice");
    #
    ...
    #        throw new ConversionException ("Failed to detect the beginning of the binary part", "CalDevice");
    #    }
    #
    ...
    #    this.checkAndAdd(CalDeviceRow.fromBin(bodis, this, attributesSeq.toArray(new String[0])));
    #    }
    ...
    def setFromMIMEFile(self, directory):
        ...
        """
        print("setFromMIME file not yet implemented for CalDeviceTable")
        return

        # java code looks like this
        # File file = new File(directory+"/CalDevice.bin");
        #
        # byte[] bytes = null;
        #
        # try {
        ...
        #         throw new ConversionException ("File " + file.getName() + " is too large", "CalDevice");
        #
        ...
        #        throw new ConversionException("Could not completely read file "+file.getName(), "CalDevice");
        #    }
        #    is.close();
        # }
        # catch (IOException e) {
    ...
    def setFromXMLFile(self, directory):
        ...
        if xmlstr.find("<BulkStoreRef") != -1:
            ...
            if self._fileAsBin:
                print(
                    "CalDevice found as XML but it should be written as binary, which is not yet implemetned. Setting to write as XML to preserve this content."
                )
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "CalDevice.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "CalDevice.xml".
        ...
        if self._fileAsBin:
            ...
            #
            # File xmlFile = new File(directory+"/CalDevice.xml");
            # if (xmlFile.exists())
            #    if (!xmlFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+xmlFile.toString()+"'", "CalDevice");
            #
            # File binFile = new File(directory+"/CalDevice.bin");
            # if (binFile.exists())
            #    if (!binFile.delete())
            #        throw new ConversionException("Problem while trying to delete a previous version of '"+binFile.toString()+"'", "CalDevice");
            #
            # try {
        ...
        #     throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalDevice");
        # }
        # catch (IOException e) {
        #      throw new ConversionException("Problem while writing the binary representation, the message was : " + e.getMessage(), "CalDevice");
        # }
