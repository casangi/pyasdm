...
class CalDeviceTable:
    ...
    def toXML(self):
        ...
        result += '\n<CalDeviceTable xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cldvc="http://Alma/XASDM/CalDeviceTable" xsi:schemaLocation="http://Alma/XASDM/CalDeviceTable http://almaobservatory.org/XML/XASDM/4/CalDeviceTable.xsd" schemaVersion="4" schemaRevision="-1">'
        result += "\n " + self._entity.toXML()
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='CalDeviceTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "CalDevice"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "CalDevice"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "CalDevice")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("numCalload")

            attributesSeq.append("calLoadNames")

            attributesSeq.append("numReceptor")

            attributesSeq.append("calEff")

            attributesSeq.append("noiseCal")

            attributesSeq.append("coupledNoiseCal")

            attributesSeq.append("temperatureLoad")

            versionStr = "2"

        else:
            # c++ and Java just assume it then must be a CalDevice table
            # this is more insistant, just in case
            if hdrdom.nodeName != "CalDeviceTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "CalDevice"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("CalDevice")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    CalDeviceRow.fromBin(eis, self, attributesSeq),
            ...
            raise ConversionException(msg, "CalDeviceTable") from None
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "CalDevice.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "CalDevice.xml")
            ...
            mimeFilePath = os.path.join(directory, "CalDevice.bin")
