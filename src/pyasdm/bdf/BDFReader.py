...
class BDFReader:
    ...
    def open(self, bdfPath):
        ...
        try:
            ...
            raise BDFReaderException(
                "Error while opening '" + bdfPath + "'. The exception was " + str(exc)
            ) from None
        # I don't think this can happen without throwing an exception
        if self._f is None:
            raise BDFReaderException(
                "Unable to open '" + bdfpath + "'. No exception was raised."
            )
    ...
    def hasSubset(self):
        ...
        result = (
            self._currentLine is not None
        ) and self._currentLine != b"--" + self._boundary_1 + b"--"
        return result
    ...
    def _requireSDMDataSubsetMIMEPart(self):
        ...
        integrationMidpoint = int(
            (sdmDataSubsetHeaderDOM.getElementsByTagName("time"))[0]
            .childNodes[0]
            .nodeValue
        )
        integrationInterval = int(
            (sdmDataSubsetHeaderDOM.getElementsByTagName("interval"))[0]
            .childNodes[0]
            .nodeValue
        )
    ...
    def printAllBinary(self):
        for binName in self._bdfHeaderData.getBinaryTypes():
            if self._bdfHeaderData.hasBinary(binName):
                ...
                if binName == "zeroLags":
                    ok = (
                        self._bdfHeaderData.getProcessorType().getName() == "CORRELATOR"
                    ) and (
                        self._bdfHeaderData.zeroLagsCorrelatorType().getName() != "FX"
                    )
                if ok:
                    print("%s:" % binName)
                    ...
                    if binName == "zeroLags":
                        print(
                            "\tcorrelatorType = %s"
                            % self._bdfHeaderData.zeroLagsCorrelatorType().getName()
                        )
                    elif binName == "autoData":
                        print(
                            "\tnormalized = %s"
                            % self._bdfHeaderData.isAutoDataNormalized()
                        )
        ...
        if len(basebandsList) > 0:
            ...
            for baseband in basebandsList:
                ...
                for spectralWindow in baseband["spectralWindows"]:
                    ...
                    print(
                        "\t\tnumSpectralPoint = %s" % spectralWindow["numSpectralPoint"]
                    )
