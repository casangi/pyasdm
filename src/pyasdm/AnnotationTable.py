...
class AnnotationTable:
    ...
    # A data structure to store the AnnotationRow s.
    # In all cases we maintain a private list of AnnotationRow s.
    _privateRows = []

    # non-temporal ASDM in Java had a private row element here to also hold  AnnotationRow s. Not needed in python.
    ...
    def autoIncrement(self, key, x):
        ...
        if key not in self._noAutoIncIds:
            ...
            # Initialize  annotationId to Tag(nextInt).
            x.setAnnotationId(Tag(nextInt, TagType.Annotation))
    ...
    def __init__(self, container, checkRowUniqueness=False):
        """
        Create a AnnotationTable attached to container.
    ...
    def checkPresenceInMemory(self):
        ...
        if not self._presentInMemory and not self._loadInProgress:
            # print("Annotation is not present in memory, setting from file")
    ...
    def add(self, x):
        ...
        x. A row to be added.
        """
        if not isinstance(x, AnnotationRow):
            raise ValueError("x must be a  AnnotationRow instance.")

        aRow = self.lookup(x.getTime(), x.getIssue(), x.getDetails())
        if aRow is not None:
            return aRow
    ...
    def newRow(self, time, issue, details):
        """
        Create a new AnnotationRow initialized to the specified values.
    ...
    def checkAndAdd(self, x, skipUniquenessCheck=False):
        ...
        if not skipUniquenessCheck:
            if self.lookup(x.getTime(), x.getIssue(), x.getDetails()) is not None:
        ...
        # is this check being skipped
        if not skipUniquenessCheck:
            if self.getRowByKey(x.getAnnotationId()) is not None:
    ...
    def get(self):
        """
        Get all rows.
        return Alls rows as a list of AnnotationRow
        """
    ...
    def fromXML(self, xmlstr):
        ...
        if (
            ...
            raise ConversionException(
                "XML is not from the expected table", "AnnotationTable"
            )
        ...
        for thisNode in tabdom.childNodes:
            ...
            if nodeName == "Entity":
                ...
                try:
                    ...
                    # reraise it as a ConversionException
                    raise ConversionException(str(exc), "AnnotationTable") from None
                    ...
                    raise ConversionException(msg, "AnnotationTable") from None
    ...
    def MIMEXMLPart(self, byteOrder):
        ...
        result += uidStr
        result += "' entityIdEncrypted='na' entityTypeName='AnnotationTable' schemaVersion='1' documentVersion='1'/>\n"
    ...
    def setFromMIME(self, byteStream):
        ...
        if loc0 < 0:
            ...
            if loc0 < 0:
                ...
                byteStream.close()
                raise ConversionException(
                    "Failed to detect the begining of the XML header.", "Annotation"
                )
            else:
        ...
        if loc1 < 0:
            byteStream.close()
            raise ConversionException(
                "Failed to detect the begining of the binary part.", "Annotation"
            )
        ...
        if not xmldom.hasChildNodes():
            byteStream.close()
            raise ConversionException("XML is not properly structured.", "Annotation")
        ...
        if hdrdom.nodeName == "ASDMBinaryTable":
            ...
            attributesSeq.append("annotationId")

            attributesSeq.append("time")

            attributesSeq.append("issue")

            attributesSeq.append("details")
            ...
            attributesSeq.append("dValue")

            attributesSeq.append("vdValue")

            attributesSeq.append("vvdValues")

            attributesSeq.append("llValue")

            attributesSeq.append("vllValue")

            attributesSeq.append("vvllValue")

            attributesSeq.append("sValue")
            ...
            # c++ and Java just assume it then must be a Annotation table
            # this is more insistant, just in case
            if hdrdom.nodeName != "AnnotationTable":
                byteStream.close()
                raise ConversionException(
                    "XML Header is not from the expected table.", "Annotation"
                )
        ...
        if numRows == -1:
            ...
            numRows = self._container.getExpectedTableSize("Annotation")
        ...
        try:
            for i in range(numRows):
                self.checkAndAdd(
                    AnnotationRow.fromBin(eis, self, attributesSeq),
    ...
    def setFromMIMEFile(self, directory):
        ...
        filename = os.path.join(directory, "Annotation.bin")
        byteStream = None
        try:
    ...
    def toFile(self, directory):
        ...
        of this (_fileAsBin==True) will be saved in a file "Annotation.bin" or
        an XML representation (_fileAsBin==False) will be saved in a file "Annotation.xml".
        ...
        if self._fileAsBin:
            ...
            xmlFilePath = os.path.join(directory, "Annotation.xml")
            ...
            mimeFilePath = os.path.join(directory, "Annotation.bin")
